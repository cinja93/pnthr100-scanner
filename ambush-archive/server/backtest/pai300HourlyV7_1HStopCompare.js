// server/backtest/pai300HourlyV7_1HStopCompare.js
// ── V7A-ORIG vs V7A-1H STOP COMPARISON ──────────────────────────────────
//
// V7A-ORIG = original V7 (weekly structural stop for new entries)
// V7A-1H   = firstHourLow - fees as initial stop for new entries
//
// ONLY difference: how the initial stop is set for brand-new Phase C entries.
// Everything else identical: $75 BE, 2-bar trailing, 1H low break,
// re-entries, lot pyramids, fees, slippage.
//
// Usage: cd server && node backtest/pai300HourlyV7_1HStopCompare.js
// ────────────────────────────────────────────────────────────────────────
...
async function main() {
  ...
  console.log('  PNTHR AMBUSH V7 — INITIAL STOP COMPARISON');
  console.log('  V7A-ORIG = weekly structural stop for new entries (original $8.7M test)');
  console.log('  V7A-1H   = firstHourLow - fees as initial stop for new entries');
  ...
  // ── SIMULATION ENGINE ──────────────────────────────────────────────────────
  function runSim({ maxPositions = 999, pessimistic = false, withdrawals = false, label = '', use1HStop = false }) {
    ...
    let skippedNo1HLow = 0;  // tracks entries skipped because no first-hour data
    let totalWithdrawn = 0;
    // Cash ledger tracking
    let minCash = NAV_INITIAL;
    let daysTappedOut = 0;    // days where cash < $500
    let peakDeployedPct = 0;  // highest deployed / NAV
      ...
      if (dayIdx >= 2) {
        ...
        for (const ticker of Object.keys(signalsByTicker)) {
          ...
          if (regime) {
            ...
            // ── STOP SELECTION: V7A-ORIG vs V7A-1H ──
            let stop;
            if (use1HStop) {
              const firstHourBars = hBars.filter(b => extractTime(b.date) < FIRST_HOUR_END);
              const firstHourLow = firstHourBars.length ? Math.min(...firstHourBars.map(b => b.low)) : null;
              ...
              stop = getWeeklyStopLong(ticker, date, ep);
            }
            ...
            if (use1HStop) {
              ...
              stop = getWeeklyStopShort(ticker, date, ep);
            }
            if (!stop || stop <= ep) continue;
    ...
    return {
      ...
      skippedNoCash, skippedMaxPos, skippedNo1HLow,
  ...
  // ── RUN COMPARISON ─────────────────────────────────────────────────────────
  console.log('\n[3] Running comparison (6 scenarios each)...\n');

  const scenarios = [
    { label: 'BASELINE',    maxPositions: 999, pessimistic: false, withdrawals: false },
    { label: 'CAP-30',      maxPositions: 30,  pessimistic: false, withdrawals: false },
  ...
  const origResults = [], newResults = [];
  for (const s of scenarios) {
    process.stdout.write(`  ORIG ${s.label}...`);
    const orig = runSim({ ...s, label: `ORIG ${s.label}`, use1HStop: false });
    origResults.push(orig);
    const origW = orig.totalWithdrawn ? `  Withdrawn: $${(orig.totalWithdrawn / 1000).toFixed(0)}K` : '';
    console.log(` CAGR: ${orig.cagr >= 0 ? '+' : ''}${orig.cagr.toFixed(1)}%  Sharpe: ${orig.sharpe.toFixed(2)}  Equity: $${Math.round(orig.equity).toLocaleString()}${origW}`);

    process.stdout.write(`  1H   ${s.label}...`);
    const n = runSim({ ...s, label: `1H ${s.label}`, use1HStop: true });
    newResults.push(n);
    const nW = n.totalWithdrawn ? `  Withdrawn: $${(n.totalWithdrawn / 1000).toFixed(0)}K` : '';
    console.log(` CAGR: ${n.cagr >= 0 ? '+' : ''}${n.cagr.toFixed(1)}%  Sharpe: ${n.sharpe.toFixed(2)}  Equity: $${Math.round(n.equity).toLocaleString()}  (no1HLow skips: ${n.skippedNo1HLow})${nW}`);
    console.log();
  }

  // ── HEAD-TO-HEAD TABLE ────────────────────────────────────────────────────
  console.log('\n' + '='.repeat(180));
  console.log('  V7A INITIAL STOP COMPARISON — ORIG (weekly structural) vs 1H (firstHourLow - fees)');
  ...
  for (let i = 0; i < scenarios.length; i++) {
    const o = origResults[i], n = newResults[i];
    // ORIG row
    console.log(`  ${('ORIG ' + scenarios[i].label).padEnd(22)} ${(o.netReturn >= 0 ? '+' : '') + Math.round(o.netReturn) + '%'} ${(o.cagr >= 0 ? '+' : '') + o.cagr.toFixed(1) + '%'} ${o.sharpe.toFixed(2).padStart(7)} ${o.sortino.toFixed(1).padStart(8)} ${(o.pf.toFixed(1) + 'x').padStart(6)} ${(o.wr.toFixed(1) + '%').padStart(6)} ${(o.payoff.toFixed(1) + 'x').padStart(7)} ${(o.maxDD.toFixed(2) + '%').padStart(7)} ${'$' + Math.round(o.equity).toLocaleString().padStart(13)} ${String(o.trades).padStart(7)} ${String(o.totalEntries).padStart(8)} ${String(o.totalReentries).padStart(8)} ${(o.totalLongEntries + 'L/' + (o.totalShortEntries) + 'S').padStart(16)} ${String(o.maxConcurrentPositions).padStart(7)} ${String(o.skippedNo1HLow).padStart(7)}`);
    // 1H row
    console.log(`  ${('1H ' + scenarios[i].label).padEnd(22)} ${(n.netReturn >= 0 ? '+' : '') + Math.round(n.netReturn) + '%'} ${(n.cagr >= 0 ? '+' : '') + n.cagr.toFixed(1) + '%'} ${n.sharpe.toFixed(2).padStart(7)} ${n.sortino.toFixed(1).padStart(8)} ${(n.pf.toFixed(1) + 'x').padStart(6)} ${(n.wr.toFixed(1) + '%').padStart(6)} ${(n.payoff.toFixed(1) + 'x').padStart(7)} ${(n.maxDD.toFixed(2) + '%').padStart(7)} ${'$' + Math.round(n.equity).toLocaleString().padStart(13)} ${String(n.trades).padStart(7)} ${String(n.totalEntries).padStart(8)} ${String(n.totalReentries).padStart(8)} ${(n.totalLongEntries + 'L/' + (n.totalShortEntries) + 'S').padStart(16)} ${String(n.maxConcurrentPositions).padStart(7)} ${String(n.skippedNo1HLow).padStart(7)}`);
    // DELTA row
    const dCagr = n.cagr - o.cagr;
    const dSharpe = n.sharpe - o.sharpe;
    const dDD = n.maxDD - o.maxDD;
    const dTrades = n.trades - o.trades;
    console.log(`    DELTA (1H-ORIG)${' '.repeat(3)} ${' '.repeat(9)} ${(dCagr >= 0 ? '+' : '') + dCagr.toFixed(1) + '%'} ${(dSharpe >= 0 ? '+' : '') + dSharpe.toFixed(2)} ${' '.repeat(8)} ${' '.repeat(6)} ${' '.repeat(6)} ${' '.repeat(7)} ${(dDD >= 0 ? '+' : '') + dDD.toFixed(2) + '%'} ${' '.repeat(14)} ${(dTrades >= 0 ? '+' : '') + String(dTrades).padStart(6)}`);
    console.log(`  ${'─'.repeat(178)}`);
  }
  ...
  for (let i = 0; i < scenarios.length; i++) {
    for (const r of [origResults[i], newResults[i]]) {
  ...
  console.log(`  ${'Scenario'.padEnd(22)} ${'Min Cash'.padStart(12)} ${'Days <$500'.padStart(11)} ${'Peak Deploy%'.padStart(13)} ${'SkipCash'.padStart(9)} ${'Final Equity'.padStart(14)} ${'Final Cash'.padStart(12)}`);
  for (let i = 0; i < scenarios.length; i++) {
    for (const r of [origResults[i], newResults[i]]) {
      const finalCash = r.equity - (r.equity * r.peakDeployedPct / 100);  // approximate
      console.log(`  ${r.label.padEnd(22)} ${'$' + Math.round(r.minCash).toLocaleString()} ${String(r.daysTappedOut).padStart(11)} ${(r.peakDeployedPct + '%').padStart(13)} ${String(r.skippedNoCash).padStart(9)} ${'$' + Math.round(r.equity).toLocaleString()} ${' '.repeat(12)}`);
    }
  }

  console.log('\n' + '='.repeat(180));
  console.log('  V7A INITIAL STOP COMPARISON COMPLETE');
  console.log('='.repeat(180));
