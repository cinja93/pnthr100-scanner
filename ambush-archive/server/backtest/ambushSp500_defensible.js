...
// (S&P 500 universe + GICS sector-EMA are defined inline below — no AI-universe imports)
...
const VITALITY_PCT         = +(process.env.VITALITY) || 0.01;  // NAV risk per trade (VITALITY env override for the risk sweet-spot sweep)
...
const MAX_LOSS             = +(process.env.MAX_LOSS) || 300;  // $ risk cap (launch-tier safety). MAX_LOSS env override; set high to size by 1% NAV (PPM rule) for the capacity study
...
const ADV_CAP_PCT          = +(process.env.ADV_CAP) || 0.02;  // §2: 2% of 20d ADV per fill (ADV_CAP env override for capacity tests)
const ADV_LOOKBACK         = 20;              // sessions
const WITHDRAWAL_THRESHOLD = process.env.NO_WITHDRAW === '1' ? Infinity : 2_000_000;  // §3 (NO_WITHDRAW=1 disables the $2M→$1M bank for the capacity sweep)
...
const SPEC = {
  ...
  reentryCooldownBars: +(process.env.REENTRY_COOLDOWN) || 0,  // bars to wait after a stop before re-entry (churn control; ~7 = 1 day)
  ...
  earningsFlat: process.env.EARNINGS_FLAT === '1',  // go flat at the close before a held name's earnings (re-enter after via normal logic)
  trailBars: +(process.env.TRAIL_BARS) || 2,        // N-bar trailing exit (>2 = hold winners longer; churn control)
  graduateThreshold: +(process.env.GRAD_TTM) || 0,  // 0=off; if a held name's trailing-12mo return ≥ this %, GRADUATE to a weekly hold (ride trenders)
};

// ── S&P 500 point-in-time universe: ticker → {sectorId, sector} + membership gate ─
const _MEM = JSON.parse(fs.readFileSync(new URL('./data/sp500Membership.json', import.meta.url).pathname, 'utf8')).members;
const AI_TICKER_META = {};
for (const t of Object.keys(_MEM)) AI_TICKER_META[t] = { sectorId: _MEM[t].sectorId, sector: _MEM[t].sector };
function getSectorName(ticker) { return AI_TICKER_META[ticker]?.sector || 'Technology'; }
// Survivorship-free gate: a name is tradeable only on dates it was actually in the index.
function wasMember(ticker, date) { const m = _MEM[ticker]; return m ? m.intervals.some(iv => date >= iv.start && date <= iv.end) : false; }
// GICS sector-optimized weekly OpEMA periods (18-26W) for the BL/SS signal gate.
const GICS_EMA = { 'Technology': 21, 'Communication Services': 21, 'Consumer Discretionary': 19, 'Consumer Staples': 18, 'Real Estate': 26, 'Utilities': 21, 'Energy': 26, 'Industrials': 24, 'Financial Services': 25, 'Healthcare': 24, 'Basic Materials': 19 };
...
// Conservative SQUARE-ROOT market impact (on top of the 5bps base slippage in price).
// Extra $ cost of taking `shares` of a name whose 20-day ADV is `adv`:
//   cost ≈ Y · σ_daily · sqrt(participation) · notional   (Y≈1, σ≈2%) — overstates cost = defensible.
// Off unless IMPACT=1. IMPACT_Y env tunes the coefficient.
const IMPACT_ON = process.env.IMPACT === '1';
const IMPACT_Y  = +(process.env.IMPACT_Y) || 1.0;
const DAILY_VOL = 0.02;
function impactCost(shares, price, adv) {
  if (!IMPACT_ON || adv <= 0 || shares <= 0) return 0;
  return shares * price * (IMPACT_Y * DAILY_VOL * Math.sqrt(shares / adv));
}
...
async function main() {
  ...
  const CLOCKHOUR = true;                 // S&P bars are true :00 clock-hour bars
  FIRST_HOUR_END = '10:00';
  const C = { daily: 'pnthr_sp500_daily_candles', weekly: 'pnthr_sp500_weekly_candles', hourly: 'pnthr_sp500_clockhour_candles' };
  console.log('  [S&P 500 · :00 clock-hour bars] first-hour window → 10:00');
  ...
  const sectorRankDocs = await db.collection('pnthr_sp500_sector_rank_daily').find({}).sort({ date: 1 }).toArray();
  const earningsDocs = await db.collection('pnthr_sp500_earnings_dates').find({}).toArray();
  const earningsExit = {}; for (const ed of earningsDocs) earningsExit[ed.ticker] = new Set(ed.exitDates || []);
  ...
  // Trailing-12-month (252 trading-day) return as of `date` — WALK-FORWARD, no look-ahead. (Trend-graduation trigger.)
  function ttmReturn(ticker, date) {
    const dates = dailyDates[ticker]; if (!dates || !dates.length) return null;
    let lo = 0, hi = dates.length; while (lo < hi) { const m = (lo + hi) >> 1; if (dates[m] <= date) lo = m + 1; else hi = m; }
    const i = lo - 1; if (i < 252) return null;
    const c1 = dailyByDate[ticker][dates[i]]?.close, c0 = dailyByDate[ticker][dates[i - 252]]?.close;
    return (c0 > 0 && c1 > 0) ? (c1 / c0 - 1) * 100 : null;
  }
  // Most recent COMPLETED weekly bar before the current week (graduated names trail off the prior week's low/high).
  function priorWeekBar(ticker, date) {
    const bars = weeklyBarsByTicker[ticker]; if (!bars || !bars.length) return null;
    const wk = getWeekOf(date);
    let lo = 0, hi = bars.length; while (lo < hi) { const m = (lo + hi) >> 1; if (bars[m].time < wk) lo = m + 1; else hi = m; }
    const i = lo - 1; return i >= 0 ? bars[i] : null;
  }
  ...
  function buildSignals(useCarnivore) {
    ...
    for (const ticker of Object.keys(weeklyBarsByTicker).filter(t => AI_TICKER_META[t])) {
      ...
      const period = GICS_EMA[getSectorName(ticker)] ?? 21;   // GICS sector-optimized weekly OpEMA (18-26W)
      const gateOffset = 0.25;                                 // V7.6 AI 1.25× daylight gate (same as the AI-300 spec run)
  ...
  function runSim(cfg) {
    ...
    let totalComm = 0, totalBorrow = 0, totalWithdrawn = 0, totalImpact = 0;
    ...
    function closePosition(ticker, pos, exitPriceSlipped, date, hour, type, adv = 0) {
      const shares = pos.totalShares; const c = comm(shares, exitPriceSlipped); totalComm += c;
      const imp = impactCost(shares, exitPriceSlipped, adv); totalImpact += imp;
      let pnl;
      if (pos.direction === 'SHORT') {
        ...
        pnl = +(shares * (pos.avgCost - exitPriceSlipped) - c - borrow - imp).toFixed(2);
        cash += +(shares * pos.avgCost + pnl).toFixed(2);
      } else {
        const proceeds = +(shares * exitPriceSlipped - c - imp).toFixed(2);
    ...
    for (let dayIdx = 0; dayIdx < tradingDates.length; dayIdx++) {
      ...
      for (let slot = 0; slot < maxSlots; slot++) {
        ...
        for (const ticker of Object.keys(positions)) {
          ...
          // ── TREND GRADUATION: once a held name's trailing-12mo return crosses the threshold,
          //    it's a confirmed strong trender → switch from the tight intraday trail to a WEEKLY
          //    hold (ride the trend; exit when the weekly signal ends). Loosen the stop on graduation
          //    so the tight trail doesn't instantly stop us out.
          if (cfg.graduateThreshold > 0 && !pos.graduated) {
            const ttm = ttmReturn(ticker, date);
            if (ttm != null && (isLong ? ttm >= cfg.graduateThreshold : ttm <= -cfg.graduateThreshold)) {
              pos.graduated = true;
              // CLEAN weekly hold: let the weekly BL→BE (long) / SS→SE (short) signal govern the exit;
              // only a WIDE disaster floor (~30%) so a normal pullback doesn't stop us out of the trend.
              pos.stop = isLong ? +(bar.close * 0.70).toFixed(2) : +(bar.close * 1.30).toFixed(2);
            }
          }
          // graduated names exit when the WEEKLY trend ends (its weekly BL/SS signal is no longer active)
          if (pos.graduated && !(isLong ? isActiveBL(ticker, date) : isActiveSS(ticker, date))) {
            const px = exitSlip(bar.close, pos.direction);
            closePosition(ticker, pos, px, date, bar.date, 'WEEKLY_EXIT', adv20ByDate[ticker]?.[date] || 0);
          ...
          if (hit) {
            ...
            const pnl = closePosition(ticker, pos, px, date, bar.date, pos.isReentry ? 'STOP_REENTRY' : 'STOP', adv20ByDate[ticker]?.[date] || 0);
            totalStopExits++;
          ...
          if (pos.nextLot <= 4) {
            ...
            if (triggered) {
              ...
              if (addShares >= 1) {
                ...
                if (cash >= lotCost) { totalComm += c; const oldBasis = pos.avgCost * pos.totalShares; cash -= lotCost; const limp = impactCost(addShares, fill, adv20ByDate[ticker]?.[date] || 0); cash -= limp; totalImpact += limp; pos.totalShares += addShares; pos.avgCost = +((oldBasis + fill * addShares) / pos.totalShares).toFixed(4); pos.nextLot++; totalLotFills++; }
              } else { pos.nextLot++; }
            }
          }
          ...
          if (pos.graduated) {
            // CLEAN weekly hold — NO intraday/weekly trailing stop. The weekly BE/SE signal governs the
            // exit (checked above); the wide disaster floor set at graduation is the only stop. Pure BL+1→BE.
          } else if (trailAllowed && extractTime(bar.date) >= FIRST_HOUR_END && bar._gi >= 1) {
            const series = hourlyAll[ticker]; const nb = cfg.trailBars || 2;
            const win = series.slice(Math.max(0, bar._gi - nb + 1), bar._gi + 1);
            if (isLong) { const trail = +(Math.min(...win.map(b => b.low)) - 0.01).toFixed(2); if (trail > pos.stop) pos.stop = trail; }
            else { const trail = +(Math.max(...win.map(b => b.high)) + 0.01).toFixed(2); if (trail < pos.stop) pos.stop = trail; }
          }
        }
        ...
        if (slot >= 1) {
          ...
          for (const ticker of universe) {
            ...
            if (!wasMember(ticker, date)) continue;   // survivorship-free: enter only while the name is in the index
            const bars = dayBars[ticker]; if (!bars || slot >= bars.length) continue;
          ...
          for (const cand of candidates) {
            ...
            { const eimp = impactCost(l1, ep, adv20ByDate[ticker]?.[date] || 0); cash -= eimp; totalImpact += eimp; }
      ...
      // ── EARNINGS-FLAT (cfg) — go flat at today's close if a held name reports after
      //    today's close (AMC) or before next open (BMO). Re-entry is the engine's normal
      //    next-day logic, gated by signal/sector/trigger ("re-enter if conditions hold").
      if (cfg.earningsFlat) {
        for (const ticker of Object.keys(positions)) {
          if (!earningsExit[ticker]?.has(date)) continue;
          const ebars = dayBars[ticker]; if (!ebars || !ebars.length) continue;
          const pos = positions[ticker]; const lastBar = ebars[ebars.length - 1];
          const px = exitSlip(lastBar.close, pos.direction);
          closePosition(ticker, pos, px, date, lastBar.date, 'EARNINGS_FLAT', adv20ByDate[ticker]?.[date] || 0);
          waiting[ticker] = { direction: pos.direction, frozenDaily: pos.frozenDaily, cycleNum: pos.cycleNum + 1, exitGi: lastBar._gi, runningLow: lastBar.low, runningHigh: lastBar.high };
    ...
    return {
      cfg, grossCurve, exits, dailyLedger, cash, banked, totalImpact,
  ...
  const {
    grossCurve, exits, dailyLedger, cash, banked, totalImpact,
    totalComm, totalBorrow, totalWithdrawn, totalNewEntries, totalReentries, totalLotFills,
    totalLongEntries, totalShortEntries, totalStopExits, totalEndExits,
    skippedNoCash, skippedNoADV, skippedSizing, worstSingleTrade,
    peakDeployed, peakWorkingNav, peakTotalValue, maxConcurrent,
  ...
  console.log('\n  FRICTIONS & FEES');
  console.log('  ' + '-'.repeat(72));
  console.log(`     Trading: commission ${fmt$(totalComm)} + borrow ${fmt$(totalBorrow)} + market impact ${fmt$(totalImpact)}`);
  ...
  const results = {
    ...
    fees: { commission: +totalComm.toFixed(2), borrow: +totalBorrow.toFixed(2), marketImpact: +totalImpact.toFixed(2), mgmt: +totalMgmtFees.toFixed(2), perf: +totalPerfFees.toFixed(2) },
  ...
  try {
    ...
    const TAG = `SP500_V76_seed${Math.round(NAV_INITIAL / 1000)}k` + (process.env.NO_WITHDRAW === '1' ? '_noWD' : '') + (process.env.EARNINGS_FLAT === '1' ? '_EF' : '');
