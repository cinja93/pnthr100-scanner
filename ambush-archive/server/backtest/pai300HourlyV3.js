// server/backtest/pai300HourlyV3.js
// ── 90-DAY HOURLY RE-ENTRY V3 — PROFIT-LOCK STOP + RE-ENTRY ───────────────
//
// Core concept: stock makes NOW/MCE, we enter L1. Runs up, hits profit threshold,
// sets PROFIT-LOCK stop (not breakeven — we KEEP the profit). Gets stopped out
// with $X locked profit. Stock trades DOWN. We wait for a REAL pullback, THEN
// look for the green hourly bar = buyers back. Re-enter at a LOWER price.
// Combined P&L = locked profit from leg 1 + re-entry trade profit from leg 2.
//
// Usage: cd server && node backtest/pai300HourlyV3.js
// ─────────────────────────────────────────────────────────────────────────
...
async function main() {
  ...
  console.log('  90-DAY HOURLY V3 — PROFIT-LOCK STOP + RE-ENTRY');
  console.log('  Lock profit at threshold, then re-enter lower after pullback');
  console.log('='.repeat(100));
      ...
      for (const [ticker, pos] of Object.entries(positions)) {
        ...
        for (const hBar of hBars) {
          ...
          if (pos.stop != null && hBar.low <= pos.stop) {
            ...
            const phase = pos.atBE ? 'PROFIT_LOCK' : 'MAX_LOSS';
            ...
            cycles.push({
              ...
              lockedProfit: pos.atBE ? +pnl.toFixed(2) : 0,
          ...
          // Profit-lock: when unrealized hits threshold, lock it in
          if (!pos.atBE && unr >= beThreshold) {
            pos.atBE = true;
            pos.lockedProfitPerShare = beThreshold / pos.l1;
            pos.stop = +(pos.entryPrice + pos.lockedProfitPerShare).toFixed(2);
          }
  ...
  for (const v of variations) {
    ...
    const beStops = cycles.filter(c => c.phase === 'PROFIT_LOCK');
    ...
    const totalLockedProfit = beStops.reduce((s, c) => s + (c.lockedProfit || 0), 0);
  ...
  const hdr = `  ${'#'.padStart(3)} ${'Lock$'.padStart(6)} ${'PB%'.padStart(5)} ${'Re-entry Style'.padEnd(32)} ${'Net$'.padStart(9)} ${'PF'.padStart(7)} ${'WR'.padStart(6)} ${'Payoff'.padStart(7)} ${'Locks'.padStart(6)} ${'Lock$Tot'.padStart(9)} ${'ReEnt'.padStart(6)} ${'Rate'.padStart(6)} ${'AvgImpr'.padStart(8)} ${'Impr%'.padStart(6)} ${'ReNet$'.padStart(8)} ${'ReWR'.padStart(6)} ${'RePF'.padStart(7)}`;
  console.log(hdr);

  for (let i = 0; i < Math.min(30, results.length); i++) {
    const r = results[i];
    console.log(`  ${String(i + 1).padStart(3)} $${String(r.be).padStart(5)} ${(r.pb * 100).toFixed(1).padStart(5)}% ${r.rsLabel.padEnd(32)} $${String(r.netPnl.toFixed(0)).padStart(8)} ${r.pf.toFixed(2).padStart(6)}x ${(r.wr * 100).toFixed(0).padStart(5)}% ${r.payoff.toFixed(1).padStart(6)}x ${String(r.beStops).padStart(6)} $${String(r.totalLockedProfit.toFixed(0)).padStart(8)} ${String(r.reentries).padStart(6)} ${(r.reentryRate * 100).toFixed(0).padStart(5)}% $${r.avgImpr.toFixed(2).padStart(7)} ${(r.posImprPct * 100).toFixed(0).padStart(5)}% $${String(r.reNetPnl.toFixed(0)).padStart(7)} ${(r.reWr * 100).toFixed(0).padStart(5)}% ${r.rePf > 100 ? '>100x' : r.rePf.toFixed(1) + 'x'}`);
  }
  ...
  console.log('='.repeat(140));
  const hdrBal = `  ${'#'.padStart(3)} ${'Lock$'.padStart(6)} ${'PB%'.padStart(5)} ${'Re-entry Style'.padEnd(32)} ${'Net$'.padStart(9)} ${'PF'.padStart(7)} ${'WR'.padStart(6)} ${'Payoff'.padStart(7)} ${'Locks'.padStart(6)} ${'Lock$Tot'.padStart(9)} ${'ReEnt'.padStart(6)} ${'Rate'.padStart(6)} ${'AvgImpr'.padStart(8)} ${'Impr%'.padStart(6)} ${'ReNet$'.padStart(8)} ${'ReWR'.padStart(6)} ${'Score'.padStart(7)}`;
  console.log(hdrBal);
