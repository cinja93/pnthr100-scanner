...
export default function AssistantLiveTable({ onNavigate, netLiquidity, onOpenChart, onAddPosition, collapsed: extCollapsed, onToggleCollapsed, headerExtra, onExitConfirmed, onPositionsSummary, hideHeader }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setData(d);
      if (d.positions) onPositionsSummary?.({ ...d.positions, recycleCandidate: d.recycleCandidate || null, recycledPositions: d.recycledPositions || [], heatReductionPlan: d.heatReductionPlan || null, sectorBreakdown: d.sectorBreakdown || [] });
      if (positionsRes.ok) {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      zIndex:        2,
                      padding:       '2px 8px',
                      background:    isLoss ? '#6f42c1' : '#16a34a',
                      border:        isLoss ? '1px solid rgba(111,66,193,0.7)' : '1px solid rgba(22,163,74,0.7)',
                      borderRadius:  4,
                      fontSize:      11,
                      fontWeight:    800,
                      color:         '#fff',
