...
import {
  fetchAiSignalHistoryWeeks,
  fetchAiSignalHistoryWeek,
  fetchAiSignalHistoryTicker,
  saveAiSignalHistorySnapshot,
  fetchAiUniverse,
  fetchEarnings,
  fetchAiMarketSnapshots,
  fetchAiEnrichedSignals,
  fetchAiClosedTrades,
  fetchAiChangelog,
  addAiChangelogEntry,
} from '../services/api';
...
function SignalArchiveTab({ weeks, onWeeksChange }) {
  ...
  useEffect(() => {
    fetchAiUniverse()
      .then(data => {
        ...
        setAiStocks(list);
        fetchEarnings(list.map(s => s.ticker)).then(setAiEarnings);
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    setTickerHistory([]);
    Promise.all([
      fetchAiSignalHistoryWeek(selectedWeek),
      fetchAiEnrichedSignals(selectedWeek).catch(() => []),
      fetchAiMarketSnapshots(selectedWeek, selectedWeek).catch(() => []),
    ])
      ...
      .catch(err => console.error('AI History week:', err))
  ...
  function handleTickerClick(ticker) {
    ...
    fetchAiSignalHistoryTicker(ticker)
      .then(setTickerHistory)
      .catch(err => console.error('AI Ticker history:', err))
      .finally(() => setTickerLoading(false));
  }

  function handleChartOpen(ticker) {
    const idx = aiStocks.findIndex(s => s.ticker === ticker);
    if (idx === -1) return;
    setChartStocks(aiStocks);
    setChartIndex(idx);
  }
  ...
  async function handleSnapshot() {
    ...
    setSnapError(false);
    try {
      const res = await saveAiSignalHistorySnapshot();
      setSnapError(false);
      setSnapMsg(`✓ Saved ${res.count} records for week of ${formatWeekOf(res.weekOf)}`);
      const w = await fetchAiSignalHistoryWeeks();
      onWeeksChange(w);
      if (!selectedWeek && w.length > 0) setSelectedWeek(w[0]);
    } catch (err) {
      setSnapError(true);
      setSnapMsg(`✗ ${err.message} — Load the AI 300 Index page first, then try again.`);
    } finally {
      setSnapping(false);
    }
  }
      ...
      <main className={styles.main}>
        ...
        {!selectedWeek && weeks.length === 0 && !weekLoading && (
          <div className={styles.emptyState}>
            ...
            <p>No AI 300 signal history yet. Hit <strong>Save This Week</strong> to capture the first snapshot.</p>
          </div>
        )}
      </main>

      {chartIndex != null && (
        <ChartModal stocks={chartStocks} initialIndex={chartIndex} earnings={aiEarnings} onClose={() => setChartIndex(null)} />
      )}
    </div>
  );
}
...
function MarketConditionsTab() {
  ...
  if (snapshots.length === 0) {
    return (
      <div style={{ padding: 40, textAlign: 'center', color: '#5a5a5a', fontSize: 14 }}>
        ...
        <p>No AI 300 market condition data yet.</p>
        <p style={{ fontSize: 12, color: '#3a3a3a' }}>Market snapshots are saved automatically by the Friday pipeline once configured.</p>
      </div>
    );
  }

  const pai300Data = filtered.map(s => ({ weekOf: s.weekOf, price: s.pai300Price, ema: s.pai300Ema21 }));
  const vixData    = filtered;
  ...
  return (
    <div style={{ padding: 24, overflowY: 'auto', flex: 1 }}>
      ...
      {chartCard('PAI300 vs 21-Week EMA',
        <SimpleLineChart
          data={pai300Data}
          lines={[
      ...
      {chartCard('VIX — Fear Index',
        <VixChart data={vixData} height={160} />
      )}
...
function DimensionLabTab() {
  ...
  if (!signals.length) {
    return (
      <div style={{ padding: 40, textAlign: 'center', color: '#5a5a5a', fontSize: 14 }}>
        <img src={pantherHead} alt="" style={{ width: 48, opacity: 0.2, marginBottom: 16 }} />
        <p>No AI 300 enriched signal data yet.</p>
...
function TradeArchiveTab() {
  ...
  if (!trades.length) {
    return (
      <div style={{ padding: 40, textAlign: 'center', color: '#5a5a5a', fontSize: 14 }}>
        <img src={pantherHead} alt="" style={{ width: 48, opacity: 0.2, marginBottom: 16 }} />
        <p style={{ color: '#5a5a5a' }}>No AI 300 closed trades yet.</p>
        <p style={{ fontSize: 12, color: '#3a3a3a' }}>Trades populate automatically when AI 300 case studies exit.</p>
      </div>
    );
  }
...
function SystemChangelogTab() {
  ...
  async function handleSubmit(e) {
    ...
    setFormErr('');
    try {
      await addAiChangelogEntry(form);
      setShowForm(false);
  ...
  return (
    <div style={{ padding: 24, overflowY: 'auto', flex: 1 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <span style={{ fontSize: 13, color: '#D4A017', fontWeight: 700 }}>AI 300 System Changelog — {entries.length} entries</span>
        <button
        ...
        <form onSubmit={handleSubmit} style={{ background: '#1a1a1a', border: '1px solid #D4A017', borderRadius: 8, padding: 20, marginBottom: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 12, marginBottom: 12 }}>
            ...
            <div>
              ...
              <input type="text" placeholder="v1.0.0" value={form.version} onChange={e => setForm(f => ({ ...f, version: e.target.value }))}
                style={{ width: '100%', background: '#111', border: '1px solid #2e2e2e', borderRadius: 4, color: '#c8c8c8', padding: '5px 8px', fontSize: 12 }} />
            </div>
            <div>
...
export default function AiSignalHistoryPage() {
  ...
  useEffect(() => {
    fetchAiSignalHistoryWeeks()
      .then(w => setWeeks(w))
      .catch(err => console.error('AI History weeks:', err));
  }, []);
    ...
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div className={styles.headerLeft}>
          ...
          <div>
            <h1 className={styles.title}>AI 300 Signal History</h1>
            <p className={styles.subtitle}>Weekly archive, market conditions, and system analytics — AI 300 universe</p>
          </div>
        </div>
      </div>
