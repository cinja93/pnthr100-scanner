  ...
  return (
    ...
    >
      ...
      <td style={{ padding: '6px 10px', textAlign: 'right' }}>
        <span style={{ color: Math.abs(o.gapPct ?? 0) >= 12 ? '#16a34a' : '#dc2626' }}>
          {o.gapPct != null ? `${o.gapPct.toFixed(1)}%` : '—'}
        </span>
        {Math.abs(o.gapPct ?? 0) < 12 && o.gapPct != null && <span style={{ color: '#dc2626', fontSize: 9, fontWeight: 700, marginLeft: 4 }}>GAP</span>}
      </td>
      <td style={{ padding: '6px 10px', textAlign: 'right' }}>
        <span style={{ color: Math.abs(o.wEmaSlope ?? 999) < 50 ? '#16a34a' : '#dc2626' }}>{o.wEmaSlope != null ? `${o.wEmaSlope.toFixed(1)}%` : '—'}</span>
    ...
    <thead>
      <tr style={{ background: '#1a1a1a', color: '#fcf000', textAlign: 'left' }}>
        ...
        <SortHeader label="Gap %"       sortKey="gapPct"     currentSort={sort} onSort={onSort} align="right"
          info={<>Distance from price to OpEMA. Threshold: 12%+ for NOW.<br/><span style={{ color: '#16a34a', fontWeight: 700 }}>Green</span> = 12%+ (qualifies)<br/><span style={{ color: '#dc2626', fontWeight: 700 }}>Red</span> = under 12% (not enough gap yet)</>} />
        <SortHeader label="Slope %"     sortKey="slope"      currentSort={sort} onSort={onSort} align="right"
          info={<>Annualized weekly EMA slope. Threshold: under 50% for NOW.<br/><span style={{ color: '#16a34a', fontWeight: 700 }}>Green</span> = under 50% (EMA flat enough)<br/><span style={{ color: '#dc2626', fontWeight: 700 }}>Red</span> = 50%+ (EMA too steep)</>} />
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11, fontFamily: 'monospace' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid #2a2a40' }}>
                  {['Ticker', 'Dir', 'Shares', 'Avg Cost', 'Price', 'Current Stop', 'New Stop', 'Move', 'Current Risk $', 'Locked P/L', 'Risk Reduced'].map(h => (
                ...
                {heatReductionPlan.adjustments.map((adj, i) => {
                  ...
                  return (
                    <tr key={adj.ticker + i} style={{ borderBottom: '1px solid #1a1a2a' }}>
                      ...
                      {(() => {
                        const currentRisk = adj.direction === 'LONG'
                          ? (adj.currentPrice - adj.currentStop) * adj.shares
                          : (adj.currentStop - adj.currentPrice) * adj.shares;
                        return (
                          <td style={{ padding: '5px 8px', color: '#dc2626' }}>${currentRisk.toFixed(2)}</td>
                        );
                      })()}
                      {(() => {
              ...
              <tfoot>
                {(() => {
                  const totalCurrentRisk = heatReductionPlan.adjustments.reduce((s, a) => {
                    return s + (a.direction === 'LONG'
                      ? (a.currentPrice - a.currentStop) * a.shares
                      : (a.currentStop - a.currentPrice) * a.shares);
                  }, 0);
                  ...
                  return (<>
                    <tr style={{ borderTop: '1px solid #2a2a40' }}>
                      <td colSpan={8} style={{ padding: '5px 8px', color: '#888', fontWeight: 700, textAlign: 'right' }}>Total current risk:</td>
                      <td style={{ padding: '5px 8px', fontWeight: 800, color: '#dc2626' }}>
                        <AumShield>${totalCurrentRisk.toFixed(2)}</AumShield>
                      </td>
                      <td colSpan={2}></td>
                    </tr>
                    <tr>
                      <td colSpan={9} style={{ padding: '5px 8px', color: '#888', fontWeight: 700, textAlign: 'right' }}>Total locked P/L:</td>
                ...
                <tr>
                  <td colSpan={10} style={{ padding: '5px 8px', color: '#888', fontWeight: 700, textAlign: 'right' }}>Total risk reduced:</td>
