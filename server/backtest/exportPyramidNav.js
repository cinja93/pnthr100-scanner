// server/backtest/exportPyramidNav.js
// ── PNTHR NAV-Scaled Pyramid Backtest — Fund Intelligence Report (Pyramid) ───
//
// Identical signal selection + pyramiding logic as exportPyramidOrders.js,
// but uses NAV-SCALED position sizing exactly matching the live Command Center:
//
//   vitality  = NAV × 1% (stocks) or NAV × 0.5% (ETFs)
//   tickerCap = NAV × 10%
//   rps       = |entryPrice − stopPrice|
//   totalShares = floor(min(floor(vitality / rps), floor(tickerCap / entryPrice)))
//
// The NAV is STATIC (does not compound) — sized as if starting fresh each trade
// at the chosen tier. This isolates strategy edge from compounding effects.
//
// Usage:  cd server && node backtest/exportPyramidNav.js [--nav 100000|500000|1000000]
//         Default: --nav 100000
//
// OUTPUT:
//   pnthr_bt_pyramid_nav_{100k|500k|1m}_trade_log — per-trade records per NAV tier
//   Console prints all metrics for the chosen NAV tier.
//
// The original exportPyramidOrders.js ($10K fixed) is UNTOUCHED.
// ─────────────────────────────────────────────────────────────────────────────
...
// ── ETF identification (matches client/src/utils/sizingUtils.js) ─────────────
...
// ── NAV-scaled position sizing (exact match of live sizePosition()) ──────────
function sizePositionNav(nav, entryPrice, stopPrice, isETF = false) {
...
async function main() {
  ...
  const outputCol  = db.collection(`pnthr_bt_pyramid_nav_${navLabel}_trade_log`);
  ...
  for (let wi = 0; wi < allWeeks.length; wi++) {
    ...
    for (const score of selected) {
      ...
      // NAV-scaled position sizing (exact match of live Command Center)
      const isETF = ETF_SET.has(ticker);
      const fullShares = sizePositionNav(STARTING_NAV, entryPrice, initStop, isETF);
      if (fullShares <= 0) continue;  // Can't size — skip
  ...
  // ── Persist to MongoDB ─────────────────────────────────────────────────────
  console.log(`Persisting ${closedTrades.length} trades to pnthr_bt_pyramid_nav_${navLabel}_trade_log...`);
  ...
  console.log('  PNTHR NAV-SCALED PYRAMID BACKTEST — FUND INTELLIGENCE REPORT (PYRAMID)');
  console.log(`  Period:         ${allWeeks[0]} → ${allWeeks[allWeeks.length - 1]}`);
  console.log(`  Starting NAV:   $${STARTING_NAV.toLocaleString()}`);
  console.log(`  Sizing:         1% vitality / 10% ticker cap / min(risk, cap)`);
  console.log(`  Pyramid:        ${LOT_PCT.map(p => (p*100).toFixed(0)+'%').join(' / ')}`);
  ...
  const totalReturn = equity > 0 ? (equity - STARTING_NAV) / STARTING_NAV * 100 : 0;
  const cagr = yearsSpan > 0 && equity > 0 ? (Math.pow(equity / STARTING_NAV, 1 / yearsSpan) - 1) * 100 : 0;
  ...
  console.log('\n' + '═'.repeat(80));
  console.log(`  INSTITUTIONAL METRICS — PYRAMID NET-OF-COSTS (NAV $${STARTING_NAV.toLocaleString()})`);
  console.log('═'.repeat(80));
  console.log(`  Starting capital:  $${STARTING_NAV.toLocaleString()}`);
