// server/ambushIrService.js
// Intelligence Report data for PNTHR Ambush V7.4 — mirrors irLiveService.js so the
// same IrLivePage renders it. Reads per-tier daily NAV + trades from
// server/data/ambushIr/{tier}.json (generated by backtest/_genAmbushIr.js) and
// REUSES the exact IR math from irLiveService.js so numbers are computed identically.
...
const _dataPath = (tier) => new URL(`./data/ambushIr/${tier}.json`, import.meta.url).pathname;
...
async function getAmbushIrData(tierKey) {
...
function getAmbushTradeLog(tierKey) {
  const tier = TIERS[tierKey];
  if (!tier) throw new Error(`Invalid tier: ${tierKey}`);
  const file = loadTierFile(tierKey);
...
export async function ambushIrMetricsHandler(req, res) {
  try {
    const data = await getAmbushIrData(req.params.tier);
    res.json(data);
  } catch (err) {
    console.error('[AMBUSH IR] Error:', err.message);
    res.status(500).json({ error: err.message });
  }
}

export async function ambushIrTradesHandler(req, res) {
  try {
    const trades = getAmbushTradeLog(req.params.tier);
    res.json({ trades, count: trades.length });
  } catch (err) {
    console.error('[AMBUSH IR] Trades error:', err.message);
    res.status(500).json({ error: err.message });
  }
}
