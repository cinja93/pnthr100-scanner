            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10, marginBottom: 24 }}>
                <MetricCard small label="Best Month" value={net?.bestMonth ? `${fmtPct(net.bestMonth.ret)}` : '—'} color={GREEN} sub={net?.bestMonth?.m}
                  info={`The single highest monthly return the fund has delivered, net of all fees. A best month this large shows the fund can capture explosive upside when the market moves in its favor. The ${fc.name} is designed to keep risk in line with the S&P 500, but its trend-following pyramid system concentrates capital into the strongest momentum names, producing outsized gains that a passive index simply cannot match.`} />
                <MetricCard small label="Worst Month" value={net?.worstMonth ? `${fmtPct(net.worstMonth.ret)}` : '—'} color={RED} sub={net?.worstMonth?.m}
                  info={`The single deepest monthly loss, net of all fees. This is the most pain an investor would have experienced in any single month. The ${fc.name} is engineered to keep worst-case drawdowns comparable to the S&P 500 through disciplined stop-loss execution and systematic position sizing. Losses are cut quickly so capital is preserved for the next opportunity. A worst month near the S&P 500's range, combined with dramatically higher returns, is the core advantage of this strategy.`} />
                <MetricCard small label="Avg Monthly Return" value={`${fmt(net?.avgMonthlyReturn)}%`} color={GREEN} sub={spy?.avgMonthlyReturn != null ? `S&P 500: ${fmt(spy.avgMonthlyReturn)}%` : undefined}
                  info={`The average gain across all months in the backtest. The S&P 500 averages roughly 0.7% per month. The ${fc.name} targets S&P-level risk but delivers multiples of that monthly return through concentrated momentum positions and a pyramid scaling system. This means your capital compounds dramatically faster month after month while experiencing a similar ride to the broad market.`} />
                <MetricCard small label="Max Drawdown (Net)" value={fmtPct(net?.maxDD)} color={RED} sub={spy?.maxDD != null ? `S&P 500: ${fmtPct(spy.maxDD)}` : undefined}
                  info={`The largest peak-to-trough decline in portfolio value after all fees. This is the deepest hole an investor would have sat through. The ${fc.name} is built to keep max drawdown in the same neighborhood as the S&P 500, which means you experience similar downside risk to owning a simple index fund, but your annual returns are multiples higher. Stops fire automatically and capital rotates into new momentum, so drawdowns are contained and recoveries are fast.`} />
                <MetricCard small label="Max Drawdown (Gross)" value={fmtPct(gross?.maxDD)} color={RED} sub={spy?.maxDD != null ? `S&P 500: ${fmtPct(spy.maxDD)}` : undefined}
                  info={`The largest peak-to-trough decline before fees are deducted. Showing this alongside the net figure lets you see how much of the drawdown comes from market moves versus fee drag. When gross and net max drawdowns are close together, it confirms that fees are not amplifying your risk. The ${fc.name} keeps both figures comparable to the S&P 500's worst pullback, giving investors confidence their downside is controlled.`} />
                <MetricCard small label="Ulcer Index" value={fmt(net?.ulcerIndex)} sub="Moderate volatility"
                  info={`Measures how deep and how long drawdowns last. Unlike standard deviation, the Ulcer Index only penalizes the painful part, the time spent underwater. A lower score means less stress on your capital and your nerves. The ${fc.name} maintains a moderate Ulcer Index because its stop-loss discipline prevents small dips from becoming deep drawdowns. You get the upside of an aggressive growth strategy with the drawdown profile of a much more conservative one.`} />
                <MetricCard small label="Monthly Std Dev" value={`${fmt(net?.monthlyStdDev)}%`}
                  info={`How much monthly returns vary from the average. The S&P 500 typically runs around 4-5% monthly standard deviation. The ${fc.name} targets a similar volatility band, which means the ride feels familiar to any investor used to owning an index fund. The difference is what you earn for that volatility. While the S&P gives you roughly 10% a year, this fund delivers multiples of that return for the same level of monthly fluctuation. You take similar risk but get paid significantly more for it.`} />
              </div>
