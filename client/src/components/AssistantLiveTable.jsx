        ...
        {data.rows.map(row => {
          ...
          return (
            <div key={row.ticker} style={cardStyle}>
              ...
              {/* Card metrics strip — bottom row, sits to the right of the
                  RSI badge. Mirrors the four numbers from PyramidCard's
                  expanded panel ($ at risk · next lot % · P&L % · P&L $)
                  so Scott can read each row's risk + P&L without expanding.
                  Server computes via assistantLiveReconcile.buildRow.metrics. */}
              {row.metrics && (
                <div
                  style={{
                    position:     'absolute',
                ...
                >
                  {row.metrics.atRisk != null && (
                    <span title="Dollar at risk = (avg − stop) × shares" style={{
                      padding: '1px 5px', borderRadius: 3,
                      background: 'rgba(252,240,0,0.10)',
                      border: '1px solid rgba(252,240,0,0.30)',
                      color: row.metrics.atRisk === 0 ? '#28a745' : '#FCF000',
                    }}>
                      ${row.metrics.atRisk >= 1000
                        ? Math.round(row.metrics.atRisk).toLocaleString()
                        : row.metrics.atRisk.toFixed(0)} risk
                    </span>
                  )}
                  {row.metrics.nextLotPct != null && (
                    <span title="Distance from current price to next unfilled lot trigger" style={{
                      padding: '1px 5px', borderRadius: 3,
                      background: 'rgba(255,215,0,0.08)',
                      border: '1px solid rgba(255,215,0,0.25)',
                      color: '#FFD700',
                    }}>
                      {Math.abs(row.metrics.nextLotPct).toFixed(1)}% to next lot
                    </span>
                  )}
                  {row.metrics.pnlPct != null && (
                    <span title="Unrealized P&L percentage" style={{
                      padding: '1px 5px', borderRadius: 3,
                      background: row.metrics.pnlPct >= 0 ? 'rgba(40,167,69,0.10)' : 'rgba(220,53,69,0.10)',
                      border: `1px solid ${row.metrics.pnlPct >= 0 ? 'rgba(40,167,69,0.35)' : 'rgba(220,53,69,0.35)'}`,
                      color: row.metrics.pnlPct >= 0 ? '#6bcb77' : '#ff6b6b',
                    }}>
                      {row.metrics.pnlPct >= 0 ? '+' : ''}{row.metrics.pnlPct.toFixed(1)}%
                    </span>
                  )}
                  {row.metrics.pnlDollar != null && (
                    <span title="Unrealized P&L dollars" style={{
                      padding: '1px 5px', borderRadius: 3,
                      background: row.metrics.pnlDollar >= 0 ? 'rgba(40,167,69,0.10)' : 'rgba(220,53,69,0.10)',
                      border: `1px solid ${row.metrics.pnlDollar >= 0 ? 'rgba(40,167,69,0.35)' : 'rgba(220,53,69,0.35)'}`,
                      color: row.metrics.pnlDollar >= 0 ? '#6bcb77' : '#ff6b6b',
                    }}>
                      {row.metrics.pnlDollar >= 0 ? '+' : '−'}${Math.abs(row.metrics.pnlDollar) >= 1000
                        ? Math.round(Math.abs(row.metrics.pnlDollar)).toLocaleString()
                        : Math.abs(row.metrics.pnlDollar).toFixed(0)}
                    </span>
                  )}
                </div>
              )}
