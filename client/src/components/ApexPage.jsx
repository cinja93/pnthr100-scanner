...
import { fetchApexStocks, createPendingEntries, API_BASE, authHeaders } from '../services/api';
import { calcHeat } from '../utils/sizingUtils.js';
import styles from './ApexPage.module.css';
import pantherHead from '../assets/panther head.png';

const QUEUE_SESSION_KEY = 'pnthr_kill_queue';
...
function QueuePanel({ queue, onRemove, onClose, onSend, sending }) {
  ...
  // Heat impact
  const nav = 100000; // fallback; real nav loaded from server but not needed for relative calc here
  ...
  // Option A candidates: weakest existing live positions to close
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', zIndex: 300,
      ...
      <div style={{ background: '#111', borderRadius: 14, border: '1px solid rgba(255,215,0,0.2)',
          ...
          {canSend ? (
            <div style={{ padding: '10px 14px', background: 'rgba(40,167,69,0.08)',
              ...
              <span style={{ color: '#28a745', fontWeight: 700, fontSize: 12 }}>
                ✅ All {items.length} positions fit within risk limits.
              </span>
            </div>
          ) : (
                ...
                <>
                  ...
                  <div style={{ padding: '10px 14px', background: 'rgba(220,53,69,0.06)',
                    ...
                    <div style={{ fontSize: 11, color: '#888', marginBottom: 6 }}>
                      Go to Command to close these first, then return here to send your queue:
                    </div>
        ...
        <div style={{ padding: '0 20px 20px', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
          ...
          <button onClick={onSend} disabled={sending || items.length === 0}
...
export default function ApexPage() {
  ...
  const [queue, setQueue]               = useState(() => {
    try {
      const saved = sessionStorage.getItem(QUEUE_SESSION_KEY);
  ...
  // ── Queue helpers ─────────────────────────────────────────────────────────
  function handleQueueToggle(item) {
    setQueue(prev => {
      ...
      try { sessionStorage.setItem(QUEUE_SESSION_KEY, JSON.stringify([...next])); } catch {}
      return next;
    });
  }

  async function handleSendToCommand() {
    if (queue.size === 0 || sendingQueue) return;
    setSendingQueue(true);
    try {
      await createPendingEntries([...queue.values()]);
      // Clear queue + session storage
      setQueue(new Map());
      try { sessionStorage.removeItem(QUEUE_SESSION_KEY); } catch {}
  ...
  return (
    <div className={styles.page} onClick={() => setInfoPopup(null)}>
      ...
      {/* ── Queue counter badge (floating bottom-right, admin only) ──────────── */}
      {isAdmin && queue.size > 0 && (
        <div
      ...
      {sendSuccess && (
        <div style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 200,
          ...
          ✓ {queue.size === 0 ? 'Entries sent to Command!' : 'Sent to Command!'}
        </div>
      )}

      {/* ── Queue Review Panel ───────────────────────────────────────────────── */}
      {showQueuePanel && isAdmin && (
        <QueuePanel
          queue={queue}
          onRemove={ticker => handleQueueToggle({ ticker, _remove: true })}
          onClose={() => setShowQueuePanel(false)}
          onSend={handleSendToCommand}
