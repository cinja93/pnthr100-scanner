#!/usr/bin/env node
/**
 * generateComplianceLetters2026.js
 *
 * Generates professional compliance letter PDFs for all overdue/due tasks,
 * uploads them to MongoDB compliance_documents, marks tasks COMPLETED,
 * and creates next recurring instances.
 *
 * Usage: cd /Users/cindyeagar/pnthr100-scanner && node scripts/generateComplianceLetters2026.js
 */
  ...
  if (t.includes('personal securities transactions')) {
    ...
    return {
      ...
      paragraphs: [
        ...
        `(a) Scott McBrien — Chief Investment Officer (Individual CRD# 2213610)\n(b) Cindy Eagar — Chief Compliance Officer / Chief Operating Officer`,
  ...
  if (t.includes('user license')) {
    return {
      ...
      paragraphs: [
        ...
        `Current Authorized Users:\n(a) Scott McBrien — Chief Investment Officer: Full access to trading platforms (Interactive Brokers), market data systems, compliance software, and firm technology infrastructure.\n(b) Cindy Eagar — Chief Compliance Officer / Chief Operating Officer: Full access to compliance systems (Comply), administrative platforms, firm email, cloud storage, and financial reporting systems.`,
  ...
  if (t.includes('trade errors')) {
    return {
      ...
      paragraphs: [
        ...
        `The firm's Trade Error Policy requires prompt identification, reporting to the CCO, documentation, and remediation of any trade errors. The policy and error log are maintained in the compliance files and are available for regulatory examination.`,
      ],
  ...
  if (t.includes('advertising') || t.includes('marketing')) {
    return {
      ...
      paragraphs: [
        ...
        `The Fund currently operates with limited marketing activity as there are no outside investors. The principals (Scott McBrien and Cindy Eagar) are the only investors in the Fund. Marketing activity and materials will be reviewed and pre-approved by the CCO before any distribution to prospective investors.`,
  ...
  if (t.includes('financial condition')) {
    return {
      ...
      paragraphs: [
        ...
        `The Fund is currently in startup/early stage with no outside investors. Capital in the Fund consists solely of investments by the principals (Scott McBrien and Cindy Eagar). The firm's financial condition will be monitored on a quarterly basis and more frequently if circumstances warrant.`,
      ],
  ...
  if (t.includes('cash movement') || t.includes('investor cash')) {
    return {
      ...
      paragraphs: [
        ...
        `(a) Scott McBrien — Chief Investment Officer\n(b) Cindy Eagar — Chief Compliance Officer / Chief Operating Officer`,
  ...
  if (t.includes('risk assessment')) {
    return {
      ...
      paragraphs: [
        ...
        `(1) Investment Risk: ${FUND_NAME} employs a systematic long-short equity strategy with quantitative risk controls including position sizing limits, sector concentration advisory (manager discretion, no hard cap per Fund policy), stop-loss mechanisms, and time-based exit rules. The investment strategy is designed to manage downside risk through disciplined entry and exit criteria. Risk Level: MODERATE — inherent in equity trading but mitigated by systematic controls.`,
  ...
  if (t.includes('complaint file')) {
    return {
      ...
      paragraphs: [
        ...
        `The firm's complaint handling policy provides for prompt acknowledgment, investigation, escalation to the CCO, documentation, and resolution of any complaints received. The complaint handling and escalation procedures remain in place, documented in the firm's Policies and Procedures Manual, and are available for regulatory examination.`,
      ],
  ...
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({
      ...
      info: {
        ...
        Author: 'Cindy Eagar, CCO — STT Capital Advisors, LLC',
    ...
    doc.moveDown(0.6);
    doc.font('Helvetica-Bold').fontSize(10.5).text('Cindy Eagar');
    doc.moveDown(0.15);
    doc.font('Helvetica').fontSize(10).text('Chief Compliance Officer / Chief Operating Officer');
    doc.moveDown(0.1);
    doc.text(FIRM_NAME);
    doc.moveDown(0.1);
    doc.text(`CRD# ${CRD}`);
    ...
    // We'll add footer text at the bottom of the current (last) page
    // For simplicity, just finalize
    doc.end();
  });
}
...
function computeNextTitle(task) {
  const title = task.title;
  const d = new Date(task.dueDate);
  const recurrence = task.recurrence;

  // For monthly blue sky, update the month in parens
  if (recurrence === 'monthly' && title.toLowerCase().includes('blue sky')) {
    const nextDate = new Date(d);
    nextDate.setUTCMonth(nextDate.getUTCMonth() + 1);
    const nextMonth = nextDate.toLocaleDateString('en-US', { month: 'short', timeZone: 'UTC' });
    // Replace month in parentheses if present, else append
    if (/\((Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\)/.test(title)) {
      return title.replace(/\((Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\)/, `(${nextMonth})`);
    }
    return `Monthly Blue Sky Filing Review (${nextMonth})`;
  }

  // For quarterly tasks, update Q label
  if (recurrence === 'quarterly') {
    const nextDate = new Date(d);
    nextDate.setUTCMonth(nextDate.getUTCMonth() + 3);
    const nextQ = getQuarterLabel(nextDate);
    const currentQ = getQuarterLabel(d);
    return title.replace(new RegExp(`${currentQ}\\b`), nextQ);
  }

  // Annual tasks keep same title
  return title;
}
...
async function main() {
  ...
  // Query tasks due on or before April 8, 2026 that are not completed
  ...
  for (const task of tasks) {
    ...
    console.log(`  Due: ${formatDate(d)} | Recurrence: ${task.recurrence || 'none'}`);
    ...
    try {
      ...
      // 4. Handle recurrence — create next instance
      if (task.recurrence) {
        const nextDue = computeNextDueDate(task);
        if (nextDue) {
          const nextTitle = computeNextTitle(task);
          const nextTask = {
            title: nextTitle,
          ...
          // Copy any other fields that exist on the task
          if (task.assignee) nextTask.assignee = task.assignee;
          if (task.priority) nextTask.priority = task.priority;
          if (task.tags) nextTask.tags = task.tags;

          // Check if next instance already exists (avoid duplicates)
          const exists = await tasksColl.findOne({
            title: nextTitle,
            dueDate: nextDue,
          });

          if (!exists) {
            await tasksColl.insertOne(nextTask);
            console.log(`  Next instance created: "${nextTitle}" due ${formatDate(nextDue)}`);
            nextInstances.push({ title: nextTitle, dueDate: nextDue });
          } else {
            console.log(`  Next instance already exists: "${nextTitle}"`);
          }
        }
      }
  ...
  console.log(`  Next instances:    ${nextInstances.length}`);
  console.log('═'.repeat(70));

  if (nextInstances.length > 0) {
    console.log('\nNew recurring task instances created:');
    for (const ni of nextInstances) {
      console.log(`  - ${ni.title} (due ${formatDate(ni.dueDate)})`);
    }
  }
