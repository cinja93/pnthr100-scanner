            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10, marginBottom: 24 }}>
                ...
                <MetricCard label="Sortino Ratio" value={fmt(net?.sortino)} color={net?.sortino >= 2 ? GREEN : '#fff'}
                  info="Measures how much money the fund makes relative to only the bad volatility, the losses. Unlike the Sharpe, big profitable moves are rewarded here, not penalized. Above 2.0 is excellent. Above 3.0 is world-class. A high Sortino gives investors peace of mind: the fund captures large gains while aggressively protecting capital on the downside. Your money works hard and stays safe." />
                <MetricCard label="Profit Factor" value={`${fmt(trades?.combined?.profitFactor)}x`} color={trades?.combined?.profitFactor >= 2 ? GREEN : '#fff'} sub="net"
                  info="For every dollar lost, this is how many dollars are made. A profit factor of 1.0x is breakeven. Most trading systems land between 1.2x and 1.8x. Above 2.0x is a dominant, highly profitable edge. At this level, the fund's winners overwhelm the losers, meaning your money compounds aggressively. This is the engine that turns capital into wealth." />
                <MetricCard label="Calmar Ratio" value={fmt(net?.calmar)}
                  info="Measures how much money you make relative to the worst dip you would have experienced. Above 1.0 means annual returns exceed the worst drawdown. Above 2.0 is outstanding. A strong Calmar saves you stress and gives you confidence to stay invested because the returns far outweigh the temporary setbacks. You are well compensated for any short-term pain." />
                <MetricCard label="Recovery Factor" value={`${fmt(net?.recoveryFactor, 0)}x`}
                  info="How many times over the fund has earned back its worst drawdown. A high recovery factor means the strategy doesn't just survive tough markets, it makes so much money afterward that the dip becomes irrelevant. This saves investors from the fear of drawdowns because the fund has proven it bounces back powerfully and keeps compounding." />
                <MetricCard label="Positive Months" value={`${net?.positivePct}%`} color={net?.positivePct >= 50 ? GREEN : '#fff'}
                  info="The percentage of months that made money. The S&P 500 is positive about 60% of months. A high positive month rate gives investors confidence in steady, reliable growth. Your account trends upward consistently rather than swinging wildly. This saves you from the emotional rollercoaster that causes most investors to sell at the worst time." />
