// server/backtest/generateInstitutionalPDF.js
// ── PNTHR Funds — Carnivore Quant Fund, LP — Institutional Tear Sheet ────────
//
// Generates a branded multi-page PDF from pnthr_bt_daily_nav + hedge metrics.
// Comprehensive: cover, exec summary, crisis alpha, annual returns, direction
// metrics, monthly heatmap, drawdown analysis, daily NAV for ALL months,
// risk architecture, rolling metrics, best/worst days, disclaimers.
//
// Usage:  cd server && node backtest/generateInstitutionalPDF.js
// ─────────────────────────────────────────────────────────────────────────────
...
async function run() {
  ...
  const maeDoc = await db.collection('pnthr_bt_mae_analysis').findOne({});
  ...
  const metricsDoc = await db.collection('pnthr_bt_hedge_metrics').findOne({});
  if (!metricsDoc) {
    console.error('No hedge metrics found in pnthr_bt_hedge_metrics.');
    console.error('Run: cd server && node backtest/computeHedgeFundMetrics.js');
    process.exit(1);
  }
  if (metricsDoc.sourceCollection !== 'pnthr_bt_pyramid_trade_log') {
    console.error(`Hedge metrics sourced from "${metricsDoc.sourceCollection}" — must be pnthr_bt_pyramid_trade_log.`);
    console.error('Run: cd server && node backtest/computeHedgeFundMetrics.js');
    process.exit(1);
  }
  ...
  const allTrades = await db.collection('pnthr_bt_pyramid_trade_log')
  ...
  const FILET_TIER = { startingCapital: 100_000, perfAlloc: 0.30, loyaltyAlloc: 0.25 };
  ...
  // Build SPY growth-of-$100K from dailyNav (monthly last-day equity)
  ...
  function drawGrowthChart(title, pnthrData, spyData, growthStats, chartY) {
    ...
    doc.fontSize(5.5).fillColor(YELLOW).text('PNTHR Fund ($100,000)', legCenterX - 92, legY, { lineBreak: false });
    // SPY legend
    doc.strokeColor(LTGRAY).lineWidth(0.75).dash(3, { space: 2 });
    doc.moveTo(legCenterX + 30, legY + 3).lineTo(legCenterX + 45, legY + 3).stroke();
    doc.undash();
    doc.fillColor(LTGRAY).text('S&P 500 ($100,000, net 0.03% ER)', legCenterX + 48, legY, { lineBreak: false });
    ...
    if (growthStats) {
      ...
      doc.fontSize(7).fillColor(YELLOW).font('Helvetica-Bold')
         .text('Filet \u2014 $100,000', leftX + 8, boxY + 5, { lineBreak: false });
      ...
      // SPY box (right)
      const spyEndNav = spyData.length ? spyData[spyData.length - 1].nav : 100000;
      const spyReturn = spyEndNav - 100000;
      const spyRetPct = ((spyEndNav / 100000 - 1) * 100).toFixed(1);
      ...
      doc.fontSize(7).fillColor(LTGRAY).font('Helvetica-Bold')
         .text('S&P 500 \u2014 $100,000', rightX + 8, boxY + 5, { lineBreak: false });
  ...
  const headlines = [
    ...
    [fmtDollar(lastDay.equity), 'Ending Equity ($100K start)', GREEN],
  ...
  const spyCagr = +((Math.pow(lastDay.spyEquity / STARTING_CAPITAL, 1 / 6.8) - 1) * 100).toFixed(1);
  const coverComp = [
    ['', 'PNTHR', 'S&P 500', 'ALPHA'],
    ...
    ['Ending Equity ($100K start)', fmtDollar(lastDay.equity), fmtDollar(lastDay.spyEquity), fmtDollar(lastDay.equity - lastDay.spyEquity)],
  ];
  ...
  const execParas = [
    ...
    `Over a rigorous 7-year backtest spanning June 2019 through April 2026, the strategy delivered a ${fmtPct(metrics.combined.net.cagr, 1)} net CAGR with a ${metrics.combined.net.sharpe.toFixed(2)} Sharpe ratio and ${metrics.combined.net.profitFactor.toFixed(1)}x profit factor, transforming $100,000 into ${fmtDollar(lastDay.equity)}. During the same period, a passive S&P 500 allocation returned ${fmtPct(spyTotalReturn, 0)}, producing ${fmtDollar(lastDay.spyEquity)}. The Fund generated ${fmtDollar(lastDay.equity - lastDay.spyEquity)} of alpha.`,
  ...
  const compRows = [
    ...
    ['Ending Equity ($100K)', fmtDollar(lastDay.equity), fmtDollar(lastDay.spyEquity), fmtDollar(lastDay.equity - lastDay.spyEquity)],
  ];
