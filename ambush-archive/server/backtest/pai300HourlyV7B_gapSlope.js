// server/backtest/pai300HourlyV7B_gapSlope.js
// ── PNTHR AMBUSH V7B — GAP/SLOPE GATE COMPARISON ─────────────────────────
//
// Identical to V7 stress test EXCEPT V7B Phase C new-MCE entries require:
//   1. Direction-aware sector gate (BL blocked in NO_GO, SS blocked in GO)
//   2. |Gap %| >= 12% (price far enough from OpEMA)
//   3. |Slope %| < 50% (EMA flat enough)
//
// This matches the live AI Orders pipeline ON DECK → NOW qualification.
// The question: does adding these gates improve or hurt the V7 results?
//
// NOTE: The original V7 sector gate checked !== 'AVOID' but tiers are
// GO/NEUTRAL/NO_GO — so the V7A sector gate was effectively a no-op.
// V7B fixes this with proper direction-aware tier gating.
//
// Runs TWO scenarios side by side:
//   V7A = original (no gap/slope, broken sector gate) — reproduces original V7
//   V7B = direction-aware sector + gap/slope          — full ON DECK → NOW funnel
//
// Usage: cd server && node backtest/pai300HourlyV7B_gapSlope.js
// ────────────────────────────────────────────────────────────────────────
...
// Gap/Slope thresholds (from aiOrdersPipeline.js qualityGrade logic)
const GAP_PCT_THRESHOLD   = 12;   // |gap%| >= 12 to qualify as BEST (NOW)
const SLOPE_PCT_THRESHOLD = 50;   // |slope%| < 50 to qualify as BEST (NOW)

// Sector tier thresholds (from aiSectorRotationService.js)
// Rank 1-6 = GO, 7-12 = NEUTRAL, 13-16 = NO_GO
// BL: allowed in GO + NEUTRAL, blocked in NO_GO
// SS: allowed in NO_GO + NEUTRAL, blocked in GO
...
async function main() {
  ...
  console.log('  PNTHR AMBUSH V7B — GAP/SLOPE GATE COMPARISON');
  console.log('  V7A = original (no gap/slope, broken sector gate)')
  console.log('  V7B = direction-aware sector + gap >= 12% + slope < 50% (ON DECK -> NOW)');
  ...
  // ── PRECOMPUTE WEEKLY EMA VALUES FOR GAP/SLOPE ─────────────────────────────
  // For each ticker, store weekly EMA values indexed by weekOf date.
  // Gap = (price - EMA) / EMA × 100
  // Slope = (EMA[current] - EMA[prior]) / EMA[prior] × 52 × 100 (annualized)
  console.log('\n[3] Precomputing weekly EMA values for gap/slope...');
  ...
  for (const ticker of allTickers) {
    ...
    for (let i = 0; i < bars.length; i++) {
      if (ema[i] != null) emaByWeek[bars[i].time] = ema[i];
    }
    weeklyEmaByTicker[ticker] = emaByWeek;
  }
  console.log(`  ${Object.keys(weeklyEmaByTicker).length} tickers with EMA values`);
  ...
  // V7A sector gate: original V7 logic (checks !== 'AVOID' — which never blocks
  // because tiers are GO/NEUTRAL/NO_GO, not AVOID)
  function getSectorOkV7A(ticker, dateStr) {
    const weekOf = getWeekOf(dateStr);
  ...
  // V7B sector gate: direction-aware, matching live aiOrdersPipeline.js
  // BL (LONG): allowed in GO + NEUTRAL, blocked in NO_GO
  // SS (SHORT): allowed in NO_GO + NEUTRAL, blocked in GO
  function getSectorOkV7B(ticker, dateStr, direction) {
    const sectorId = AI_TICKER_META[ticker]?.sectorId;
    ...
    const tier = aiSectorTierByDate[best]?.[sectorId];
    if (!tier) return true; // no tier data → pass through

    if (direction === 'LONG' && tier === 'NO_GO') return false;
    if (direction === 'SHORT' && tier === 'GO') return false;
    return true;
  }
  ...
  // ── GAP/SLOPE CHECK ────────────────────────────────────────────────────────
  // Matches aiOrdersPipeline.js computeGapAndSlope() logic.
  // Uses the daily bar's close (or open for entry-day estimates) vs the weekly
  // EMA value for the current week. Returns { gapPct, slopePct } or null.
  function getGapAndSlope(ticker, dateStr) {
    const weekOf = getWeekOf(dateStr);
    const emaMap = weeklyEmaByTicker[ticker];
    if (!emaMap) return null;

    // Find the most recent weekly EMA <= this date's week
    let emaVal = emaMap[weekOf];
    if (emaVal === undefined) {
      const weeks = Object.keys(emaMap).sort();
      let best = null;
      for (const w of weeks) { if (w <= weekOf) best = w; else break; }
      if (!best) return null;
      emaVal = emaMap[best];
    }
    if (!emaVal || emaVal <= 0) return null;

    // Use the daily bar close for the current price
    const dailyBar = dailyBarMap[ticker]?.[dateStr];
    if (!dailyBar) return null;
    const price = dailyBar.close;

    const gapPct = ((price - emaVal) / emaVal) * 100;

    // Slope: need the prior week's EMA
    const weeks = Object.keys(emaMap).sort();
    const weekIdx = weeks.indexOf(weekOf);
    let slopePct = null;
    if (weekIdx >= 1) {
      const priorEma = emaMap[weeks[weekIdx - 1]];
      if (priorEma && priorEma > 0) {
        slopePct = ((emaVal - priorEma) / priorEma) * 52 * 100;
      }
    }

    return { gapPct, slopePct };
  }

  function passesGapSlopeGate(ticker, dateStr) {
    const gs = getGapAndSlope(ticker, dateStr);
    if (!gs) return false; // no data = fail (conservative)
    const absGap = Math.abs(gs.gapPct);
    const absSlope = Math.abs(gs.slopePct ?? 999);
    return absGap >= GAP_PCT_THRESHOLD && absSlope < SLOPE_PCT_THRESHOLD;
  }

  // ── SIMULATION ENGINE ──────────────────────────────────────────────────────
  // useGapSlope = true for V7B, false for V7A
  // useSectorGate = true for V7B/V7C (direction-aware sector), false for V7A (broken gate)
  function runSim({ maxPositions = 999, pessimistic = false, withdrawals = false, label = '', useGapSlope = false, useSectorGate = false }) {
    ...
    let skippedGapSlope = 0; // V7B diagnostic — how many entries gap/slope filtered out
    let skippedSector = 0;   // V7B diagnostic — how many entries sector gate filtered out
    ...
    for (let dayIdx = 0; dayIdx < hourlyTradingDates.length; dayIdx++) {
      ...
      for (const [ticker, w] of Object.entries(waiting)) {
        ...
        // V7A uses original sector gate; V7B/V7C uses direction-aware gate
        if (useSectorGate ? !getSectorOkV7B(ticker, date, w.direction) : !getSectorOkV7A(ticker, date)) continue;
      ...
      if (dayIdx >= 2) {
        ...
        for (const ticker of Object.keys(signalsByTicker)) {
          ...
          // Determine direction from regime before sector gate
          const mcDirection = regime ? 'LONG' : 'SHORT';

          // Sector gate: V7A uses original (broken AVOID check); V7B/V7C uses direction-aware GO/NO_GO
          if (useSectorGate) {
            if (!getSectorOkV7B(ticker, date, mcDirection)) { skippedSector++; continue; }
          } else {
            if (!getSectorOkV7A(ticker, date)) continue;
          }

          // ┌──────────────────────────────────────────────────────────────────┐
          // │  V7B ADDITIONS: Gap/Slope + direction-aware sector gate         │
          // │  Matches live AI Orders pipeline: ON DECK → NOW qualification   │
          // │  Gap >= 12%, Slope < 50%, sector tier aligned with direction    │
          // └──────────────────────────────────────────────────────────────────┘
          if (useGapSlope && !passesGapSlopeGate(ticker, date)) {
            skippedGapSlope++;
            continue;
          }
    ...
    return {
      ...
      skippedNoCash, skippedMaxPos, skippedGapSlope, skippedSector,
      longEntries: totalLongEntries, shortEntries: totalShortEntries,
    };
  }

  // ── RUN COMPARISON ─────────────────────────────────────────────────────────
  console.log('\n[4] Running V7A vs V7C comparison (3 scenarios each)...\n');
  console.log('  V7A = original (no sector gate, no gap/slope)');
  console.log('  V7C = direction-aware sector gate ONLY (no gap/slope)\n');

  const configs = [
    { label: 'BASELINE',     maxPositions: 999, pessimistic: false, withdrawals: false },
    { label: 'CAP-40',       maxPositions: 40,  pessimistic: false, withdrawals: false },
    { label: 'WORST-CASE',   maxPositions: 40,  pessimistic: true,  withdrawals: true  },
  ];

  const resultsA = [], resultsC = [];

  for (const cfg of configs) {
    process.stdout.write(`  V7A ${cfg.label}...`);
    const rA = runSim({ ...cfg, label: `V7A ${cfg.label}`, useGapSlope: false, useSectorGate: false });
    resultsA.push(rA);
    console.log(` CAGR: ${rA.cagr >= 0 ? '+' : ''}${rA.cagr.toFixed(1)}%  Sharpe: ${rA.sharpe.toFixed(2)}  Equity: $${Math.round(rA.equity).toLocaleString()}`);

    process.stdout.write(`  V7C ${cfg.label}...`);
    const rC = runSim({ ...cfg, label: `V7C ${cfg.label}`, useGapSlope: false, useSectorGate: true });
    resultsC.push(rC);
    console.log(` CAGR: ${rC.cagr >= 0 ? '+' : ''}${rC.cagr.toFixed(1)}%  Sharpe: ${rC.sharpe.toFixed(2)}  Equity: $${Math.round(rC.equity).toLocaleString()}  (sector filtered: ${rC.skippedSector})`);
    console.log();
  }

  // ── RESULTS TABLE ─────────────────────────────────────────────────────────
  console.log('\n' + '='.repeat(160));
  console.log('  V7A vs V7C HEAD-TO-HEAD — SECTOR GATE ONLY');
  console.log('  V7A = No sector gate (original V7 — produced $8.7M)');
  console.log('  V7C = Direction-aware sector gate (BL blocked in NO_GO, SS blocked in GO)');
  console.log('='.repeat(160));
  console.log(`  ${'Scenario'.padEnd(22)} ${'Net Ret'.padStart(9)} ${'CAGR'.padStart(8)} ${'Sharpe'.padStart(7)} ${'Sortino'.padStart(8)} ${'PF'.padStart(6)} ${'WR'.padStart(6)} ${'Payoff'.padStart(7)} ${'MaxDD'.padStart(7)} ${'Equity'.padStart(14)} ${'Trades'.padStart(7)} ${'Entries'.padStart(8)} ${'L/S'.padStart(10)} ${'MaxPos'.padStart(7)} ${'SecSkip'.padStart(8)}`);
  console.log(`  ${'─'.repeat(158)}`);

  for (let i = 0; i < configs.length; i++) {
    const rA = resultsA[i], rC = resultsC[i];
    const fmtRow = (r) =>
      `  ${r.label.padEnd(22)} ${(r.netReturn >= 0 ? '+' : '') + r.netReturn.toFixed(0) + '%'} ${(r.cagr >= 0 ? '+' : '') + r.cagr.toFixed(1) + '%'} ${r.sharpe.toFixed(2).padStart(7)} ${r.sortino.toFixed(1).padStart(8)} ${(r.pf.toFixed(1) + 'x').padStart(6)} ${(r.wr.toFixed(1) + '%').padStart(6)} ${(r.payoff.toFixed(1) + 'x').padStart(7)} ${(r.maxDD.toFixed(2) + '%').padStart(7)} ${'$' + Math.round(r.equity).toLocaleString()} ${String(r.trades).padStart(7)} ${String(r.totalEntries).padStart(8)} ${(r.longEntries + 'L/' + r.shortEntries + 'S').padStart(10)} ${String(r.maxConcurrentPositions).padStart(7)} ${String(r.skippedSector).padStart(8)}`;
    console.log(fmtRow(rA));
    console.log(fmtRow(rC));
    // Delta row
    const dCagr = rC.cagr - rA.cagr;
    const dSharpe = rC.sharpe - rA.sharpe;
    const dPF = rC.pf - rA.pf;
    const dWR = rC.wr - rA.wr;
    const dMaxDD = rC.maxDD - rA.maxDD;
    const dTrades = rC.trades - rA.trades;
    const dEntries = rC.totalEntries - rA.totalEntries;
    const winner = rC.cagr > rA.cagr ? 'V7C' : 'V7A';
    const sharpeWinner = rC.sharpe > rA.sharpe ? 'V7C' : 'V7A';
    console.log(`  ${'  DELTA (C-A)'.padEnd(22)} ${''.padStart(9)} ${(dCagr >= 0 ? '+' : '') + dCagr.toFixed(1) + '%'} ${(dSharpe >= 0 ? '+' : '') + dSharpe.toFixed(2)} ${''.padStart(8)} ${(dPF >= 0 ? '+' : '') + dPF.toFixed(1)} ${(dWR >= 0 ? '+' : '') + dWR.toFixed(1) + '%'} ${''.padStart(7)} ${(dMaxDD >= 0 ? '+' : '') + dMaxDD.toFixed(2) + '%'} ${''.padStart(14)} ${(dTrades >= 0 ? '+' : '') + String(dTrades)} ${(dEntries >= 0 ? '+' : '') + String(dEntries)} ${''.padStart(10)} ${''.padStart(7)} ${'CAGR→' + winner + ' Sharpe→' + sharpeWinner}`);
    console.log(`  ${'─'.repeat(158)}`);
  }
  ...
  console.log(`\n  ACTIVITY BREAKDOWN`);
  console.log(`  ${'─'.repeat(140)}`);
  console.log(`  ${'Scenario'.padEnd(22)} ${'Entries'.padStart(8)} ${'Re-entry'.padStart(9)} ${'1H Exits'.padStart(9)} ${'Trail'.padStart(6)} ${'BEStops'.padStart(8)} ${'Lots'.padStart(6)} ${'SkipCash'.padStart(9)} ${'SkipCap'.padStart(8)} ${'SecSkip'.padStart(8)} ${'Comm'.padStart(10)} ${'Borrow'.padStart(8)} ${'Worst$'.padStart(9)}`);
  for (const r of [...resultsA, ...resultsC]) {
    console.log(`  ${r.label.padEnd(22)} ${String(r.totalEntries).padStart(8)} ${String(r.totalReentries).padStart(9)} ${String(r.totalExits1H).padStart(9)} ${String(r.totalTrailingExits).padStart(6)} ${String(r.totalBEStopUps).padStart(8)} ${String(r.totalLotFills).padStart(6)} ${String(r.skippedNoCash).padStart(9)} ${String(r.skippedMaxPos).padStart(8)} ${String(r.skippedSector).padStart(8)} ${'$' + Math.round(r.totalComm).toLocaleString()} ${'$' + Math.round(r.totalBorrow).toLocaleString()} ${'$' + Math.round(r.worstSingleTrade).toLocaleString()}`);
  }

  console.log('\n' + '='.repeat(160));
  console.log('  V7C SECTOR GATE COMPARISON COMPLETE');
  console.log('='.repeat(160));
