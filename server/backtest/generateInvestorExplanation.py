...
def S(name, **kw):
    ...
    if key not in _style_cache:
        _style_cache[key] = ParagraphStyle(name, **kw)
...
def build():
    ...
    s.append(hr())
    s.append(Paragraph(
        'This document explains — step by step, in sequential order — exactly how the PNTHR AI Elite Fund '
    ...
    headline_data = [
        ['', 'Gross', 'Net ($1M Wagyu)', 'Net ($100K Filet)', 'SPY'],
        ['CAGR', '67.26%', '53.36%', '46.94%', '7.75%'],
        ['Total Return', '+840%', '+544%', '+435%', '+37%'],
        ['$100K Becomes', '$940K', '$644K', '$535K', '$137K'],
        ['$1M Becomes', '$9.40M', '$6.44M', '—', '$1.37M'],
        ['Max Drawdown', '-20.49%', '-23.82%', '-25.87%', '-25.36%'],
        ['Sharpe Ratio', '1.67', '1.39', '1.24', '0.31'],
    ]
    hl_tbl = Table(headline_data, colWidths=[1.3*inch, 1.1*inch, 1.3*inch, 1.3*inch, 1.1*inch])
    hl_style = [
        ('BACKGROUND', (0, 0), (-1, 0), HexColor('#1a1a1a')),
        ('BACKGROUND', (0, 1), (-1, -1), CELL_BG),
        ('GRID', (0, 0), (-1, -1), 0.5, HexColor('#333333')),
    ...
    s.append(Paragraph('<i>Backtest period: January 3, 2022 through May 13, 2026 (4.36 years). '
        ...
        '1,619 closed trades.</i>', note_style()))
    s.append(PageBreak())
    ...
    s.append(Paragraph(
        '<b>1.</b> Weekly close is above the stock\'s Optimized EMA (OpEMA) — confirms the trend is up<br/>'
        '<b>2.</b> The OpEMA slope is rising (this week\'s EMA &gt; last week\'s) — trend is accelerating<br/>'
        '<b>3.</b> Weekly close broke above the highest high of the prior 2 completed weeks by at least $0.01 — breakout confirmation<br/>'
        '<b>4.</b> The stock is in the "daylight zone" — the weekly low is between 1% and 10% above the EMA. '
    ...
    s.append(Paragraph(
        ...
        body_style()))
    s.append(Paragraph(
        'For re-entries on the same side (BE then a new BL), there is no upper cap — the trend is already proven. '
    ...
    s.append(Paragraph(
        ...
        '<b>Entry gate:</b> Stock must be at least 1.25x the EMA value (the relaxed gate — gives AI stocks room to run)<br/>'
    ...
    s.append(Paragraph(
        ...
        'We ran a head-to-head backtest (Nov 2022 - May 2026) on every overlap ticker individually under both '
    ...
    s.append(Paragraph(
        ...
        body_style()))
    s.append(Paragraph(
        'Names like TSLA, META, ARM, ETN, ORCL, IBM — they run under tighter rules because the data '
    ...
    s.append(Paragraph(
        ...
        '(4) Carnivore tickers enter pre-qualified — no regime gate, no sector rotation re-check.',
        body_style()))
    ...
    s.append(Paragraph(
        '<b>For longs:</b> The HIGHER of (2-week low minus $0.01) or (entry close minus ATR). Higher = tighter = more conservative.<br/>'
        '<b>For shorts:</b> The LOWER of (2-week high plus $0.01) or (entry close plus ATR). Lower = tighter.',
        body_style()))
    ...
    s.append(Paragraph(
        ...
        'market stress — <b>every single approach destroyed returns:</b>',
        body_style()))
    ...
    s.append(Paragraph(
        ...
        'dozens of configurations tested and rejected.',
        body_style()))
    ...
    s.append(make_table(
        ...
        [
            ...
            ['Total Return', '+840%', '+544%', '+435%', '+37%'],
            ['Sharpe Ratio', '1.67', '1.39', '1.24', '0.31'],
            ['Sortino Ratio', '2.87', '2.36', '2.07', '—'],
            ['Max Drawdown', '-20.49%', '-23.82%', '-25.87%', '-25.36%'],
            ['Calmar Ratio', '3.28', '2.24', '1.81', '—'],
            ['Recovery Factor', '41x', '23x', '17x', '—'],
        ],
    ...
    s.append(Paragraph(
        ...
        '2.47x the size of losers (profit factor). SPY beta: 0.96 — similar market exposure but multiples of the return. '
    ...
    eq_tbl = Table(
        [[Paragraph(f'<b><font color="#fcf000">{r[0]}</font></b>', S('eql', fontSize=9.5, fontName='Helvetica-Bold'))
          if i == 0 else Paragraph(f'<font color="#cccccc">{r[0]}</font>', S('eqr', fontSize=9.5))
          for i, _ in [(0, None)]][0] if False else  # dummy
         [Paragraph(f'<font color="#cccccc">{c}</font>', S(f'eqc{j}', fontSize=9.5, alignment=TA_CENTER))
          for j, c in enumerate(r)]
         for r in [['', 'Wagyu ($1M)', 'Filet ($100K)']] + eq_data],
        colWidths=[2.2*inch, 1.8*inch, 1.8*inch]
    )
    # Rebuild properly
    eq_header = [
    ...
    s.append(Paragraph(
        'That is the system. Every number verified against the code. Every claim backed by a backtest that models '
        'real-world execution, not theoretical same-bar fills. Every alternative tested and rejected with data.',
        body_style()))

    s.append(Spacer(1, 20))
    s.append(hr())
    s.append(Paragraph(
        '<i>PNTHR AI Elite Fund. Built by traders. Proven by data.</i>',
        S('closer', fontSize=10, fontName='Helvetica-BoldOblique', textColor=YELLOW, alignment=TA_CENTER)))
