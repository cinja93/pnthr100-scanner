...
// Shared LONG-vs-SHORT scorecard — long (Elite paper) vs short (legacy book)
// on real data. Rendered on the Elite AI page.
  ...
  return (
    <div style={{ margin: '0 0 12px' }}>
      <div style={{ fontSize: 11, fontWeight: 800, color: '#9a9aa6', letterSpacing: 0.5, marginBottom: 5 }}>
        LONG vs SHORT SCORECARD <span style={{ color: '#666', fontWeight: 400 }}>· long vs short on real data{sc.from ? ` · ${sc.from}→${sc.to}` : ''}</span>
      </div>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 250, padding: '9px 13px', borderRadius: 8, background: '#16161c', border: '1px solid #2a2a33', borderLeft: '3px solid #ef4444' }}>
          <div style={{ fontSize: 11, fontWeight: 800, color: '#ef4444', letterSpacing: 0.4, marginBottom: 4 }}>SHORT · legacy book</div>
          <div style={{ fontSize: 12, color: '#aaa', fontFamily: 'monospace' }}>
            {sc.short.n} trades · win {sc.short.wr}% · <b style={{ color: col(sc.short.pnl) }}>{sign(sc.short.pnl)}{fmtUsd(sc.short.pnl)}</b>
          </div>
        </div>
      </div>
      {sc.ambLong?.n > 0 && <div style={{ fontSize: 10, color: '#777', marginTop: 5 }}>Legacy longs (migrating to Elite): {sc.ambLong.n} · win {sc.ambLong.wr}% · {fmtUsd(sc.ambLong.pnl)}</div>}
    </div>
  );
}
