...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  // TREE comparison read live from the frozen baseline so it never goes stale on a relock.
  let treeStr = 'n/a';
  try {
    const fs = await import('fs');
    const tb = JSON.parse(fs.readFileSync(new URL('./data/treeProjectionBaseline.json', import.meta.url), 'utf8'));
    const tm = tb.metrics || {};
    treeStr = `+${Math.round(tm.netReturnPct)}%/${tm.cagrPct}% CAGR/-${Number(tm.maxDDPct).toFixed(1)}% DD`;
  } catch { /* baseline unreadable — leave n/a */ }
  console.log(`  vs page +235% (non-comp) / -68.8% (wrong set) · vs TREE (most-liquid) ${treeStr}`);
  process.exit(0);
}
