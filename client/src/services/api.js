...
export async function fetchShortStocks(forceRefresh = false) {
  try {
    const params = forceRefresh ? `?refresh=1&_=${Date.now()}` : '';
    const response = await fetch(`${API_BASE}/api/stocks/shorts${params}`);
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  } catch (error) {
...
export async function fetchRankingByDate(date) {
  try {
    const response = await fetch(`${API_BASE}/api/rankings/${date}`);
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  } catch (error) {
...
export async function fetchStockHistory(ticker) {
  try {
    const response = await fetch(`${API_BASE}/api/stock-history/${ticker}`);
