...
Adapted from the Carnivore Quant Fund PPM v6.10 design template.
Same legal structure (Delaware LP, Rule 506(c), Section 3(c)(1)),
same fee schedule, same service providers. AI 300-specific universe
description, strategy parameters, and regime gate methodology.
...
MIDDLE_FOOTER_TEXT  = ("PNTHR FUNDS  \u00b7  PNTHR AI ELITE 300 FUND  \u00b7  "
                       "CONFIDENTIAL  \u00b7  June 2026  \u00b7  pnthrfunds.com")
FOOTER_BRAND_LEFT   = "PNTHR FUNDS:"
FOOTER_DOC_BREADCRUMB = "PNTHR AI Elite 300 Fund  |  Private Placement Memorandum"
...
def build_cover():
    ...
    story.append(P("PNTHR AI Elite 300 Fund, LP", COVER_TITLE_WHITE))
    ...
    story.append(P("PNTHR AI Elite 300 Fund, LP", TITLE_STYLE))
    story.append(Spacer(1, 0.2 * inch))
    ...
    story.append(P(
        ...
        "evaluation of a proposed investment in PNTHR FUNDS, PNTHR AI Elite 300 Fund, LP, "
...
NOTICES_PARAS = [
    ...
    "&ldquo;INTERESTS&rdquo;) IN PNTHR FUNDS, PNTHR AI ELITE 300 FUND, LP (THE &ldquo;FUND&rdquo;) "
...
def build_executive_summary():
    ...
    story.append(P(
        "PNTHR FUNDS, PNTHR AI Elite 300 Fund, LP (the &ldquo;Partnership&rdquo; or the "
    ...
    story.append(P(
        ...
        "engine that integrates multi-timeframe trend analysis across three levels: "
        "(i) the individual security&rsquo;s price structure, (ii) its AI sub-sector "
        "trend as measured by a synthetic sector index, and (iii) the broad AI market "
        "regime as measured by PAI300. Each sub-sector uses an exponential moving "
        "average period independently calibrated to that sub-sector&rsquo;s "
        "characteristic volatility and trend duration. Qualifying signals are ranked "
        "by a proprietary multi-factor scoring model, and the highest-ranked candidates "
        "are deployed through a five-lot pyramid structure with disciplined stop-loss "
        "and position-risk rules. The specific parameters, formulas, and thresholds "
        "that constitute the PNTHR Signal System are proprietary and are not disclosed "
        "in this Memorandum. The Fund&rsquo;s investment strategy is described in "
        "greater detail in Section IV (Investment Strategy) and Section V (Investment "
        "Process).",
        BODY))
    ...
    story.append(P(
        "Capital preservation is a core pillar. The Fund risks no more than 1% of Net "
        "Asset Value per equity position (0.5% per ETF position), constrains total per-"
        "ticker exposure to 10% of Net Asset Value, and operates under a disciplined "
        "stop-loss system that ratchets in the direction of the trade. The Fund may, at "
        "the Investment Manager&rsquo;s discretion, take opportunistic positions of "
        "greater size when specific intraday conditions warrant, provided that such "
        "positions are not held overnight absent signal confirmation.",
        BODY))
    ...
    story.append(bullet(
        ...
        "disciplined position sizing, stop-loss execution, and macro/sector regime gates;"))
    ...
    story.append(bullet(
        ...
        "proprietary PNTHR AI 300 Index (PAI300) serves as both the regime gate and "
        "performance benchmark, ensuring that entry and exit decisions reflect conditions "
        "within the AI sector itself rather than diluted signals from the broader equity "
        "market;"))

    story.append(bullet(
        "<b>Each AI sub-sector is tuned independently.</b> The Signal System applies a "
        "sector-optimized moving-average framework calibrated to each sub-sector&rsquo;s "
        "characteristic trend duration and volatility, rather than applying a single "
        "generic lookback across all holdings;"))

    story.append(bullet(
        "<b>The signal engine has been developed through multiple years of research and "
        "out-of-sample refinement,</b> applying AI-sector regime analysis, proprietary "
        "sub-sector trend alignment, and single-name momentum to surface long and short "
        "opportunities across all sixteen sub-sectors;"))

    story.append(bullet(
        "<b>Rigorous risk discipline governs every position.</b> The five-lot pyramid, "
        "per-position heat caps, a 10% single-name concentration limit, and the PNTHR "
        "Proprietary Stop Loss System (PPSLS) are designed to preserve capital and limit "
        "drawdowns regardless of market conditions."))
...
def build_summary_of_terms():
    ...
    rows = [
        ("Partnership",
         "PNTHR FUNDS, PNTHR AI Elite 300 Fund, LP, a Delaware limited partnership."),
        ("General Partner",
...
def build_investment_opportunity():
    ...
    story.append(P(
        ...
        "Fund&rsquo;s sub-sector regime gates and systematic signal framework are "
        "designed to adapt to these rotations faster than discretionary approaches.",
        BODY))
...
def build_investment_strategy():
    ...
    story.append(P(
        ...
        "systematic long/short equity approach built on the PNTHR Signal System "
        "(the &ldquo;Signal System&rdquo;), a proprietary signal-generation and "
        "execution framework developed by the Investment Manager specifically for the "
        "artificial-intelligence equity ecosystem. The Strategy applies rules-based "
        "combinations of multi-timeframe trend analysis, AI sub-sector regime evaluation, "
        "multi-factor scoring, and disciplined position sizing to the PNTHR AI 300 "
        "Universe. The Strategy was developed and confirmed through multiple years of "
        "historical backtesting and ongoing out-of-sample research across AI-sector "
        "market cycles, including the 2022-2023 AI drawdown and the subsequent AI "
        "infrastructure buildout, and is the exclusive investment strategy of the Fund.",
        BODY))
    ...
    story.append(P("MULTI-TIMEFRAME TREND AND AI SUB-SECTOR ANALYSIS", H2))

    story.append(P(
        "Each security in the Universe is evaluated weekly using a proprietary "
        "multi-timeframe trend-momentum framework. The evaluation integrates three "
        "distinct levels of trend context:",
        BODY))

    story.append(bullet(
        "<b>Security Trend.</b> The individual trend character of the security itself, "
        "measured against a proprietary moving-average framework. Each security&rsquo;s "
        "moving-average period is assigned based on the AI sub-sector to which it "
        "belongs, reflecting the observation that different segments of the AI value "
        "chain exhibit different characteristic trend durations (for example, AI "
        "semiconductor equities may trend on different cycles than enterprise SaaS or "
        "AI biotech names)."))

    story.append(bullet(
        "<b>Sector Trend.</b> The trend character of the security&rsquo;s AI sub-sector, "
        "measured against a proprietary synthetic sector index constructed by the "
        "Investment Manager. The Strategy assigns each security to one of sixteen (16) "
        "proprietary AI sub-sectors and evaluates the corresponding synthetic sector "
        "index against its own proprietary moving-average framework. Each AI sub-sector "
        "uses a sector-optimized exponential moving average period calibrated to the "
        "sub-sector&rsquo;s characteristic volatility and trend duration."))

    story.append(bullet(
        "<b>Broad-Market Trend.</b> The trend character of the broader AI equity "
        "market, measured against the PNTHR AI 300 Index (&ldquo;PAI300&rdquo;), a "
        "proprietary capped market-capitalization-weighted index of all securities in "
        "the Universe maintained by the Investment Manager."))

    story.append(P("AI SUB-SECTOR ROTATION", H2))

    story.append(P(
        "The Strategy incorporates a proprietary AI sub-sector rotation framework that "
        "evaluates the relative strength of each of the sixteen AI sub-sectors on a "
        "weekly basis. Sub-sectors exhibiting confirmed bullish trend momentum receive "
        "priority allocation from the scoring model, while sub-sectors in deteriorating "
        "or bearish regimes are deprioritized or targeted for short exposure. This "
        "rotation mechanism is designed to concentrate capital in the strongest-performing "
        "segments of the AI value chain at any given time, rather than maintaining static "
        "allocation across all sub-sectors.",
        BODY))

    story.append(P(
        "The framework produces two primary directional signals:",
        BODY))

    story.append(bullet(
        "<b>Buy Long (&ldquo;BL&rdquo;) Signal.</b> A qualifying long setup for a "
        "security exhibiting confirmed upward trend momentum, structural breakout "
        "characteristics, and alignment with the bullish trend of its AI sub-sector "
        "and the broader AI market as measured by PAI300. BL signals require that the "
        "security&rsquo;s price has confirmed above a proprietary threshold relative "
        "to its sector-optimized moving average."))

    story.append(bullet(
        "<b>Sell Short (&ldquo;SS&rdquo;) Signal.</b> A qualifying short setup for a "
        "security exhibiting confirmed downward trend momentum, structural breakdown "
        "characteristics, and alignment with deterioration in its AI sub-sector and "
        "the broader AI market. SS Signals are subject to additional macro and "
        "sub-sector confirmation filters described below and are generated only when "
        "the PAI300 and the security&rsquo;s sub-sector are both in confirmed bearish "
        "regimes."))

    story.append(P("MULTI-FACTOR SCORING MODEL", H2))

    story.append(P(
        "Each qualifying signal is evaluated and ranked using the Investment "
        "Manager&rsquo;s proprietary multi-factor scoring model (internally designated "
        "&ldquo;PNTHR APEX&rdquo;). The scoring model assigns each signal a composite "
        "score reflecting the strength of the opportunity across several factor "
        "categories, including AI-market regime alignment, AI sub-sector trend "
        "alignment, signal quality, signal freshness, relative momentum within the "
        "security&rsquo;s AI sub-sector, and corroboration from shorter-timeframe "
        "indicators. The specific factors, weights, and mathematical formulation of "
        "the scoring model are proprietary to the Investment Manager and are not "
        "disclosed in this Memorandum.",
        BODY))

    story.append(P(
        "Each week, the Investment Manager applies the scoring model to all qualifying "
        "BL and SS Signals in the Universe, ranks the resulting signals by composite "
        "score, and selects the highest-ranked candidates for execution subject to the "
        "entry and risk-management rules described below.",
        BODY))

    story.append(P("MACRO AND AI SUB-SECTOR REGIME GATES", H2))

    story.append(P(
        "Signals are subject to two regime gates that suspend new entries when the "
        "AI market or sub-sector environment is not favorable to the signal direction:",
        BODY))

    story.append(bullet(
        "<b>BL Regime Gate.</b> No new BL entries are initiated while the PAI300 "
        "is in a confirmed bearish trend regime. Existing BL "
        "positions are not affected by the BL Regime Gate and continue to operate under "
        "the Strategy&rsquo;s position-level exit rules."))

    story.append(bullet(
        "<b>SS Crash Gate.</b> No new SS entries are initiated unless the PAI300 "
        "and the security&rsquo;s AI sub-sector index both exhibit confirmed "
        "bearish trend conditions together with negative short-term return "
        "characteristics, in each case per the Investment Manager&rsquo;s proprietary "
        "thresholds. The SS Crash Gate is intended to limit short exposure to confirmed "
        "macro and sector deterioration."))

    story.append(P("FIVE-LOT PYRAMID ENTRY SYSTEM", H2))

    story.append(P(
        "Each entry is sized using a five-lot pyramid structure. Full position size "
        "is calibrated to the Fund&rsquo;s per-position risk limit (described below), "
        "and capital is deployed incrementally as the position moves favorably. The "
        "allocation of the full position across the five lots is as follows:",
        BODY))

    pyr_hdr = ParagraphStyle(
        name="pyr_hdr", fontName="Helvetica-Bold", fontSize=10, leading=12,
        alignment=TA_LEFT, spaceBefore=0, spaceAfter=0,
    )
    pyr_cell = ParagraphStyle(
        name="pyr_cell", fontName="Helvetica", fontSize=10, leading=12,
        alignment=TA_LEFT, spaceBefore=0, spaceAfter=0,
    )

    def pH(t):
        return Paragraph(t, pyr_hdr)

    def pC(t):
        return Paragraph(t, pyr_cell)

    pyramid_rows = [
        [pH("Lot"), pH("% of Full Position"), pH("When Deployed"), pH("Stop Treatment Upon Fill")],
        [pC("Lot 1"), pC("35%"), pC("Initial entry at Signal confirmation"),
         pC("Initial stop set per the PPSLS")],
        [pC("Lot 2"), pC("25%"), pC("Upon favorable movement beyond a proprietary threshold (after time gate)"),
         pC("Stop ratcheted in direction of trade")],
        [pC("Lot 3"), pC("20%"), pC("Upon further favorable movement"),
         pC("Stop ratcheted in direction of trade")],
        [pC("Lot 4"), pC("12%"), pC("Upon further favorable movement"),
         pC("Stop ratcheted in direction of trade")],
        [pC("Lot 5"), pC("8%"), pC("Upon further favorable movement"),
         pC("Stop ratcheted in direction of trade")],
    ]
    ptbl = Table(pyramid_rows, colWidths=[0.55 * inch, 0.85 * inch, 2.80 * inch, 1.80 * inch])
    ...
    story.append(P(
        "The pyramid structure is designed to deploy the majority of a position&rsquo;s "
        "capital only after the trade has proven itself by moving in the intended "
        "direction, thereby limiting the downside exposure of the initial lot and "
        "progressively locking in gains as each additional lot is filled. The specific "
        "favorable-movement thresholds required to trigger each subsequent lot, the "
        "applicable time gate, and the specific price levels to which stops are "
        "ratcheted upon each lot fill are proprietary to the Investment Manager and "
        "are not disclosed herein.",
        BODY))

    story.append(P("THE PNTHR PROPRIETARY STOP LOSS SYSTEM (PPSLS)", H2))

    story.append(P(
        "Every open position is managed under the PNTHR Proprietary Stop Loss System "
        "(the &ldquo;PPSLS&rdquo;). The PPSLS applies two independent stop-loss "
        "methodologies at each weekly update and selects the more conservative of the "
        "two as the governing stop:",
        BODY))

    story.append(bullet(
        "<b>Volatility-Based Ratchet.</b> A volatility-adjusted trailing stop that "
        "moves only in the direction of the trade (upward for long positions, "
        "downward for short positions) and never against the trade."))

    story.append(bullet(
        "<b>Structural Range Stop.</b> A price-level stop anchored to the recent "
        "trading range of the security. The structural stop also moves only in the "
        "direction of the trade."))

    story.append(P(
        "The PPSLS never moves a stop against the direction of an open trade. Stops "
        "lock in profits progressively as the trade works. The specific lookback "
        "periods, volatility coefficients, and structural anchor rules used by the "
        "PPSLS are proprietary and are not disclosed in this Memorandum.",
        BODY))

    story.append(P(
        "In addition to the PPSLS ratchet, the Fund applies a twenty (20)-trading-day "
        "stale-position exit that closes any position that remains unprofitable for "
        "twenty (20) consecutive trading days following initial entry.",
        BODY))

    story.append(P("POSITION SIZING AND RISK LIMITS", H2))

    story.append(P(
        "Position sizing is calibrated on a per-position basis by reference to the "
        "distance between entry price and the initial stop (&ldquo;risk per share&rdquo;). "
        "Each equity position is sized such that the product of share count and risk "
        "per share does not exceed 1.00% of the Fund&rsquo;s Net Asset Value. ETF "
        "positions are sized to a maximum of 0.50% of Net Asset Value. No single "
        "security, regardless of direction, may represent more than 10.00% of the "
        "Fund&rsquo;s Net Asset Value at entry.",
        BODY))
...
def build_investment_process():
    ...
    story.append(P(
        "The Fund&rsquo;s investment process operates on a weekly signal cycle anchored "
        "to the close of U.S. equity trading on each Friday (or the immediately preceding "
        "trading day if a Friday is a market holiday). Each week, the Investment Manager "
    ...
    story.append(P(
        "At the weekly close, the Signal System evaluates each of the approximately "
        "303 securities in the PNTHR AI 300 Universe against the Strategy&rsquo;s "
        "proprietary BL and SS criteria, evaluates each security&rsquo;s AI sub-sector "
        "regime, computes the composite APEX score for each qualifying signal, and "
        "applies the PAI300 regime gate and sub-sector regime gates described in "
        "Section IV. The output of this step is a ranked pool of Strategy-eligible "
        "trade candidates staged for execution the following Monday.",
        BODY))

    story.append(P("2. PRE-TRADE SCORING AND REVIEW (PNTHR ANALYZE)", H2))

    story.append(P(
        "Each candidate surviving signal generation is scored a second time under the "
        "Investment Manager&rsquo;s pre-trade review framework (&ldquo;PNTHR "
        "Analyze&rdquo;), which evaluates setup quality, risk profile, entry conditions, "
        "AI sub-sector concentration, portfolio fit, and wash-sale compliance. PNTHR "
        "Analyze produces a final ranking that informs the specific trades selected "
        "for execution.",
        BODY))

    story.append(P("3. ENTRY CONFIRMATION (PNTHR COMMAND CENTER)", H2))

    story.append(P(
        "Selected trades are queued in the Investment Manager&rsquo;s internal command "
        "workflow for final review, position sizing, and execution preparation. At this "
        "stage, position size is calculated per the 1.00% equity vitality cap / 0.50% "
        "ETF cap, and per-ticker exposure is validated against the 10.00% ticker cap. "
        "The Investment Manager confirms each trade prior to transmission to the "
        "Fund&rsquo;s prime broker.",
        BODY))
    ...
    story.append(P("5. POSITION MANAGEMENT AND PYRAMID DEPLOYMENT", H2))

    story.append(P(
        "Following Lot 1 entry, each open position is managed weekly under the PPSLS "
        "and the five-lot pyramid entry system described in Section IV. Stops are "
        "updated weekly; subsequent lots are deployed only when the position has moved "
        "favorably by the required threshold and any applicable time gate has cleared. "
        "Positions that trigger the PPSLS stop or the 20-trading-day stale-hunt exit "
        "are closed at the Investment Manager&rsquo;s discretion during the following "
        "trading session.",
        BODY))
...
def build_management():
    ...
    story.append(bullet(
        "<b>Investment Selection.</b> Select trades from the ranked signal pool "
        "generated by the PNTHR Signal System, subject to pre-trade review, risk "
        "limits, and regime gates."))
    story.append(bullet(
        "<b>Risk Management.</b> Implement per-position sizing, portfolio-level "
        "exposure limits, the PPSLS stop system, the 20-trading-day stale-hunt exit, "
        "and wash-sale compliance."))
    story.append(bullet(
    ...
    story.append(bullet(
        "<b>Monitoring and Review.</b> Continuously monitor open positions, signal "
        "decay, and macro/sector regime state, and adjust portfolio exposure "
        "accordingly."))
    story.append(bullet(
...
def build_detailed_summary_of_terms():
    ...
    story.append(row("Partnership",
        "PNTHR FUNDS, PNTHR AI Elite 300 Fund, LP, a Delaware limited partnership (the "
...
def build():
    doc = SimpleDocTemplate(
        ...
        title="PNTHR Funds, PNTHR AI Elite 300 Fund, LP - Private Placement Memorandum v1.0",
